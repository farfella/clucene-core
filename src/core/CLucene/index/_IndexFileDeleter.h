...
private:
    ...
    */
    ...
    public:
      ...
      const std::vector<std::wstring>& getFileNames();
...
private:
    ...
    typedef CL_NS(util)::CLHashMap<wchar_t*, RefCount*,
...
public:
    ...
    void decRef(const std::wstring& fileName);
    RefCount* getRefCount(const wchar_t * fileName);
     ...
     */
    IndexFileDeleter(CL_NS(store)::Directory* directory, IndexDeletionPolicy* policy, SegmentInfos* segmentInfos, std::wostream* infoStream, DocumentsWriter* docWriter);
    ~IndexFileDeleter();
     ...
     * that segment.
     */
    void refresh(const wchar_t* segmentName);
    void refresh();
    void close();
    ...
    void CLUCENE_LOCAL_DECL incRef(const std::vector<std::wstring>& files);
    void CLUCENE_LOCAL_DECL decRef(const std::vector<std::wstring>& files);
    void CLUCENE_LOCAL_DECL decRef(SegmentInfos* segmentInfos);
    void CLUCENE_LOCAL_DECL deleteFiles(std::vector<std::wstring>& files);

    /** Delets the specified files, but only if they are new
     *  (have not yet been incref'd). */
    void CLUCENE_LOCAL_DECL deleteNewFiles(const std::vector<std::wstring>& files);
    void CLUCENE_LOCAL_DECL deleteFile(const wchar_t * fileName);
};
