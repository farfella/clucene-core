...
FieldInfo::FieldInfo(	const wchar_t* _fieldName, 
...
void FieldInfos::addIndexed(const wchar_t** names, const bool storeTermVectors, const bool storePositionWithTermVector,
...
void FieldInfos::add(const wchar_t** names,const bool isIndexed, const bool storeTermVectors,
...
FieldInfo* FieldInfos::add( const wchar_t* name, const bool isIndexed, const bool storeTermVector,
...
FieldInfo* FieldInfos::addInternal( const wchar_t* name, const bool isIndexed, const bool storeTermVector,
...
int32_t FieldInfos::fieldNumber(const wchar_t* fieldName)const {
	FieldInfo* fi = fieldInfo(fieldName);
	return (fi!=NULL) ? fi->number : -1;
}

FieldInfo* FieldInfos::fieldInfo(const wchar_t* fieldName) const {
	FieldInfo* ret = byName.get(fieldName);
	return ret;
}

const wchar_t* FieldInfos::fieldName(const int32_t fieldNumber) const {
...
void FieldInfos::write(IndexOutput* output) const{
	...
	for (size_t i = 0; i < size(); ++i) {
	    ...
	    output->writeString(fi->name,wcslen(fi->name));
...
void FieldInfos::read(IndexInput* input) {
	...
	for (int32_t i = 0; i < size; ++i){
	    wchar_t* name = input->readString(); //we could read name into a string buffer, but we can't be sure what the maximum field length will be.
		bits = input->readByte();
