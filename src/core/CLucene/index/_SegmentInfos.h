...
public:
    ...
    std::wstring name;									// unique name in dir
...
private:
    ...
    std::vector<std::wstring> _files;                               // cached list of files that this segment uses
                                                  // in the Directory
    ...
    std::wstring docStoreSegment;					  // name used to derive fields/vectors file we share with
                                                  // other segments
    ...
    * files this segment has. */
    ...
    void addIfExists(std::vector<std::wstring>& files, const std::wstring & fileName);

public:
    SegmentInfo(const wchar_t * _name, const int32_t _docCount, CL_NS(store)::Directory* _dir,
        bool _isCompoundFile = SegmentInfo::CHECK_DIR,
        bool _hasSingleNormFile = false,
        int32_t _docStoreOffset = -1,
        const wchar_t * _docStoreSegment = NULL,
    ...
    */
    ...
    std::wstring getDelFileName() const;
    ...
    */
    std::wstring getNormFileName(const int32_t number) const;
    ...
    */
    const std::wstring& getDocStoreSegment() const;
    ...
    *  has the same dir and same name. */
    ...
    /** Used for debugging */
    std::wstring segString(CL_NS(store)::Directory* dir);
};
...
public:
    ...
    */
    static int64_t getCurrentSegmentGeneration(std::vector<std::wstring>& files);
    ...
    * @param files -- array of file names to check
    */
    static std::wstring getCurrentSegmentFileName(std::vector<std::wstring>& files);
    ...
    * @param directory -- directory to search for the latest segments_N file
    */
    static std::wstring getCurrentSegmentFileName(CL_NS(store)::Directory* directory);

    /**
    * Get the segments_N filename in use by this segment infos.
    */
    std::wstring getCurrentSegmentFileName();

    /**
    * Parse the generation off the segments file name and
    * return it.
    */
    static int64_t generationFromSegmentsFileName(const wchar_t* fileName);

    /**
    * Get the next segments_N filename that will be written.
    */
    std::wstring getNextSegmentFileName();
...
*/
    ...
    */
    void read(CL_NS(store)::Directory* directory, const wchar_t * segmentFileName);
    ...
    */
    ...
    {
    protected:
        const wchar_t * fileDirectory;
        CL_NS(store)::Directory* directory;

        void doRun();
        virtual bool tryDoBody(const wchar_t* segmentFileName, CLuceneError& ret_err) = 0;
    };
    ...
    */
    ...
    {
    protected:
        virtual RET doBody(const wchar_t * segmentFileName) = 0;
        RET result;

        //catch only IO errors, return true on success...
        bool tryDoBody(const wchar_t * segmentFileName, CLuceneError& ret_err)
        {
            try
            {
    ...
    public:
        ...
        FindSegmentsFile(const wchar_t * dir)
        {
            this->directory = NULL;
    ...
    public:
        ...
        FindSegmentsVersion(const wchar_t * dir);
        int64_t doBody(const wchar_t * segmentFileName);
    };
    ...
    public:
        ...
        FindSegmentsRead(const wchar_t * dir, SegmentInfos* _this);
        bool doBody(const wchar_t * segmentFileName);
    };
