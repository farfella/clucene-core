...
private:
	...
	int32_t binarySearch(const CL_NS(util)::ArrayBase<wchar_t*>& array, const wchar_t* key) const;
public:
	//note: termFreqs must be the same length as terms
	SegmentTermVector(const wchar_t* field, CL_NS(util)::ArrayBase<wchar_t*>* terms,
    CL_NS(util)::ArrayBase<int32_t>* termFreqs);
	...
	*/
	...
	int32_t indexOf(const wchar_t* termText);
	CL_NS(util)::ArrayBase<int32_t>* indexesOf(const CL_NS(util)::ArrayBase<wchar_t*>& termNumbers, const int32_t start, const int32_t len);
...
public:
	void get(const int32_t docNum, const wchar_t* field, TermVectorMapper* mapper);
	...
	*/
	TermFreqVector* get(const int32_t docNum, const wchar_t* field);
...
private:
	CL_NS(util)::ObjectArray<SegmentTermVector>* readTermVectors(const int32_t docNum,
		const wchar_t** fields, const int64_t* tvfPointers, const int32_t len);

	void readTermVectors(const wchar_t** fields, const int64_t* tvfPointers,
	...
	* @throws IOException
	*/
	void readTermVector(const wchar_t* field, const int64_t tvfPointer, TermVectorMapper* mapper);
...
public:
	SegmentTermPositionVector(const wchar_t* field,
	  CL_NS(util)::ArrayBase<wchar_t*>* terms,
	  CL_NS(util)::ArrayBase<int32_t>* termFreqs,
	...
	*/
	...
	int32_t indexOf(const wchar_t* termText){ return SegmentTermVector::indexOf(termText); }
  CL_NS(util)::ArrayBase<int32_t>* indexesOf(const CL_NS(util)::ArrayBase<wchar_t*>& termNumbers, const int32_t start, const int32_t len);
...
public:
	...
	*/
	virtual void setExpectations(const wchar_t* _field, const int32_t numTerms, const bool storeOffsets,
	...
	*/
	virtual void map(const wchar_t* term, const int32_t termLen, const int32_t frequency,
...
public:
	...
	void setExpectations(const wchar_t* _field, const int32_t numTerms,
		const bool storeOffsets, const bool storePositions);

	void map(const wchar_t* term, const int32_t termLen, const int32_t frequency,
		CL_NS(util)::ArrayBase<TermVectorOffsetInfo*>* _offsets,
