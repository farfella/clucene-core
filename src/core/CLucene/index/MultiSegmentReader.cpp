    ...
    normsCache(NormsCacheType(true, true))
{
    ...
    std::map<std::wstring, size_t> segmentReaders;
    if (oldReaders != NULL)
    {
    ...
    for (int32_t i = infos->size() - 1; i >= 0; i--)
    {
        ...
        map<std::wstring, size_t>::iterator oldReaderIndex = segmentReaders.find(infos->info(i)->name);
    ...
    if (oldNormsCache != NULL)
    {
        ...
        while (it != oldNormsCache->end())
        {
            ...
            for (size_t i = 0; i < subReaders->length; i++)
            {
                std::map<std::wstring, size_t>::iterator oldReaderIndex = segmentReaders.find(((SegmentReader*) (*subReaders)[i])->getSegmentName());
...
const std::wstring MultiTermEnum::getObjectName() const { return getClassName(); }
const std::wstring MultiTermEnum::getClassName() { return L"MultiTermEnum"; }
...
TermFreqVector* MultiSegmentReader::getTermFreqVector(int32_t n, const wchar_t* field)
{
    ensureOpen();
    int32_t i = readerIndex(n);        // find segment num
    return (*subReaders)[i]->getTermFreqVector(n - starts[i], field);
}
void MultiSegmentReader::getTermFreqVector(int32_t docNumber, const wchar_t* field, TermVectorMapper* mapper)
{
    ensureOpen();
...
uint8_t* MultiSegmentReader::norms(const wchar_t* field)
{
    SCOPED_LOCK_MUTEX(THIS_LOCK)
        ensureOpen();
    uint8_t* bytes;
    bytes = normsCache.get((wchar_t*) field);
    if (bytes != NULL)
    {
...
void MultiSegmentReader::norms(const wchar_t* field, uint8_t* result)
{
    SCOPED_LOCK_MUTEX(THIS_LOCK)
        ensureOpen();
    uint8_t* bytes = normsCache.get((wchar_t*) field);
...
void MultiSegmentReader::doSetNorm(int32_t n, const wchar_t* field, uint8_t value)
{
    normsCache.removeitr(normsCache.find((wchar_t*) field));                         // clear cache
    int32_t i = readerIndex(n);                           // find segment num
...
int32_t MultiSegmentReader::readerIndex(int32_t n) const
{    // find reader for doc n:
    return readerIndex(n, this->starts, this->subReaders->length);
}


int32_t MultiSegmentReader::readerIndex(const int32_t n, int32_t* starts, int32_t numSubReaders)
{	  // find reader for doc n:
...
bool MultiSegmentReader::hasNorms(const wchar_t* field)
{
    ensureOpen();
    for (size_t i = 0; i < subReaders->length; i++)
    {
    ...
    for (size_t i = 0; i < subReaders->length; i++)
    {
        ...
        while (itr != temp.end())
        {
            if (hashList.find(*itr) == hashList.end())
                hashList.insert(_wcsdup(*itr));
            itr++;
        }
    }
    //move the items into the return
    CLHashList<wchar_t*>::iterator itr = hashList.begin();
...
const std::wstring MultiSegmentReader::getClassName()
{
    return L"MultiSegmentReader";
}
const std::wstring MultiSegmentReader::getObjectName() const
{
    return getClassName();
}
...
{
    CND_PRECONDITION(current != NULL, L"current==NULL, check that next() was called");
...
void MultiTermDocs::seek(Term* tterm)
{
    ...
    CND_PRECONDITION(tterm != NULL, L"tterm is NULL");
...
MultiTermEnum::MultiTermEnum(ArrayBase<IndexReader*>* subReaders, const int32_t *starts, const Term* t)
{
    ...
    CND_PRECONDITION(starts != NULL, L"starts is NULL");
    ...
    CND_CONDITION(queue != NULL, L"Could not allocate memory for queue");
...
TermDocs* MultiTermPositions::termDocs(IndexReader* reader)
{
    ...
    CND_CONDITION(ret != NULL,
        L"Dynamic downcast in MultiTermPositions::termDocs from"
        L" TermPositions to TermDocs failed."
    );
    return ret;
}
...
int32_t MultiTermPositions::nextPosition()
{
    ...
    //Pre  - current != NULL
    //Post -
    CND_PRECONDITION(current != NULL, L"current is NULL");

    TermPositions* curAsTP = current->__asTermPositions();

    CND_CONDITION(curAsTP != NULL,
        L"Dynamic downcast in MultiTermPositions::nextPosition from"
        L" SegmentTermDocs to TermPositions failed."
    )
