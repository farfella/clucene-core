...
class SegmentReader: public DirectoryIndexReader {
  ...
  typedef CL_NS(util)::CLHashtable<const wchar_t*,Norm*,
    CL_NS(util)::Compare::WChar, CL_NS(util)::Equals::WChar,
...
protected:
  ...
  void doUndeleteAll();
  void commitChanges();
  void doSetNorm(int32_t doc, const wchar_t* field, uint8_t value);

  // can return null if norms aren't stored
  uint8_t* getNorms(const wchar_t* field);
...
public:
  ...
  uint8_t* norms(const wchar_t* field);
   ...
   * @throws IOException
   */
  TermFreqVector* getTermFreqVector(int32_t docNumber, const wchar_t* field=NULL);

  void getTermFreqVector(int32_t docNumber, const wchar_t* field, TermVectorMapper* mapper);
