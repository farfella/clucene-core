...
TermFreqVector* MultiReader::getTermFreqVector(int32_t n, const wchar_t* field)
{
    ensureOpen();
    int32_t i = readerIndex(n);        // find segment num
...
void MultiReader::getTermFreqVector(int32_t docNumber, const wchar_t* field, TermVectorMapper* mapper)
{
    ensureOpen();
...
uint8_t* MultiReader::norms(const wchar_t* field)
{
    SCOPED_LOCK_MUTEX(THIS_LOCK)
        ensureOpen();
    uint8_t* bytes;
    bytes = _internal->normsCache.get((wchar_t*) field);
    if (bytes != NULL)
    {
...
void MultiReader::norms(const wchar_t* field, uint8_t* result)
{
    SCOPED_LOCK_MUTEX(THIS_LOCK)
...
void MultiReader::doSetNorm(int32_t n, const wchar_t* field, uint8_t value)
{
    _internal->normsCache.removeitr(_internal->normsCache.find((wchar_t*) field));                         // clear cache
    int32_t i = readerIndex(n);                           // find segment num
...
int32_t MultiReader::readerIndex(const int32_t n) const
{	  // find reader for doc n:
    return MultiSegmentReader::readerIndex(n, this->starts, this->subReaders->length);
}

bool MultiReader::hasNorms(const wchar_t* field)
{
    ensureOpen();
    for (size_t i = 0; i < subReaders->length; i++)
    {
...
const std::wstring MultiReader::getClassName()
{
    return L"MultiReader";
}
const std::wstring MultiReader::getObjectName() const
{
    return getClassName();
}
CL_NS_END
