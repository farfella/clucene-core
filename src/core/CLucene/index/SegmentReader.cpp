...
SegmentReader::Norm::Norm(IndexInput* instrm, bool _useSingleNormStream, int32_t n, int64_t ns, SegmentReader* r, const wchar_t * seg) :
    number(n),
    normSeek(ns),
    _this(r),
    segment(seg),
...
void SegmentReader::initialize(SegmentInfo* si, int32_t readBufferSize, bool doOpenStores, bool doingReopen)
{
    ...
    try
    {
        ...
        {
            cfsReader = _CLNEW CompoundFileReader(directory(), (segment + L"." + IndexFileNames::COMPOUND_FILE_EXTENSION).c_str(), readBufferSize);
            cfsDir = cfsReader;
        }
        ...
        if (doOpenStores)
        {
            if (si->getDocStoreOffset() != -1)
            {
                ...
                {
                    storeCFSReader = _CLNEW CompoundFileReader(directory(), (si->getDocStoreSegment() + L"." + IndexFileNames::COMPOUND_FILE_STORE_EXTENSION).c_str(), readBufferSize);
        ...
        _fieldInfos = _CLNEW FieldInfos(cfsDir, (segment + L".fnm").c_str());

        std::wstring fieldsSegment;
        ...
        if (doOpenStores)
        {
            ...
            {
                std::wstring err = L"doc counts differ for segment ";
                err += si->name;
                err += L": fieldsReader shows ";
                err += fieldsReader->size();
                err += L" but segmentInfo shows ";
                err += si->docCount;
        ...
        // make sure that all index files have been read or are kept open
        // so that if an index update removes them we'll still have them
        freqStream = cfsDir->openInput((segment + L".frq").c_str(), readBufferSize);
        proxStream = cfsDir->openInput((segment + L".prx").c_str(), readBufferSize);
        openNorms(cfsDir, readBufferSize);

        if (doOpenStores && _fieldInfos->hasVectors())
        { // open term vector files only as needed
            std::wstring vectorsSegment;
            if (si->getDocStoreOffset() != -1)
...
void SegmentReader::doDelete(const int32_t docNum)
{
    ...
    SCOPED_LOCK_MUTEX(THIS_LOCK)
        ...
        CND_PRECONDITION(docNum >= 0, L"docNum is a negative number");
    CND_PRECONDITION(docNum < maxDoc(), L"docNum is bigger than the total number of documents");
    ...
    if (deletedDocs == NULL)
    {
        ...
        CND_CONDITION(deletedDocs != NULL, L"No memory could be allocated for deletedDocs");
    }
...
void SegmentReader::files(vector<std::wstring>& retarray)
{
    ...
    std::vector<std::wstring> tmp = si->files();
...
TermEnum* SegmentReader::terms()
{
    ...
    CND_PRECONDITION(tis != NULL, L"tis is NULL");
...
TermEnum* SegmentReader::terms(const Term* t)
{
    ...
    CND_PRECONDITION(t != NULL, L"t is NULL");
    CND_PRECONDITION(tis != NULL, L"tis is NULL");
...
bool SegmentReader::document(int32_t n, Document& doc, const FieldSelector* fieldSelector)
{
    ...
    CND_PRECONDITION(n >= 0, L"n is a negative number");
...
void SegmentReader::getFieldNames(FieldOption fldOption, StringArrayWithDeletor& retarray)
{
    ...
    for (size_t i = 0; i < len; i++)
    {
        ...
        if (v)
            retarray.push_back(_wcsdup(fi->name));
    }
}

bool SegmentReader::hasNorms(const wchar_t* field)
{
    ensureOpen();
...
void SegmentReader::norms(const wchar_t* field, uint8_t* bytes)
{
    ...
    CND_PRECONDITION(bytes != NULL, L"field is NULL");
...
uint8_t* SegmentReader::getNorms(const wchar_t* field)
{
    SCOPED_LOCK_MUTEX(THIS_LOCK)
        Norm* norm = _norms.get(field);
...
uint8_t* SegmentReader::norms(const wchar_t* field)
{
...
void SegmentReader::doSetNorm(int32_t doc, const wchar_t* field, uint8_t value)
{
    Norm* norm = _norms.get(field);
...
std::wstring SegmentReader::SegmentName(const wchar_t * ext, const int32_t x)
{
...
void SegmentReader::openNorms(Directory* cfsDir, int32_t readBufferSize)
{
    ...
    for (size_t i = 0; i < _fieldInfos->size(); i++)
    {
        ...
        if (fi->isIndexed && !fi->omitNorms)
        {
            ...
            std::wstring fileName = si->getNormFileName(fi->number);
            ...
            std::wstring ext = std::wstring(L".") + IndexFileNames::NORMS_EXTENSION;
...
TermFreqVector* SegmentReader::getTermFreqVector(int32_t docNumber, const wchar_t* field)
{
    ensureOpen();
    if (field != NULL)
    {
...
void SegmentReader::getTermFreqVector(int32_t docNumber, const wchar_t* field, TermVectorMapper* mapper)
{
    ensureOpen();
    FieldInfo* fi = _fieldInfos->fieldInfo(field);
    if (fi == NULL || !fi->storeTermVector || termVectorsReaderOrig == NULL)
...
void SegmentReader::loadDeletedDocs()
{
    ...
    if (hasDeletions(si))
    {
        ...
        {
            std::wstring err = L"number of deletes (";
            err += deletedDocs->count();
            err += L") exceeds max doc (";
            err += maxDoc();
            err += L") for segment ";
            err += si->name;
            _CLTHROWT(CL_ERR_CorruptIndex, err.c_str());
    ...
    try
    {
        ...
        if (!normsUpToDate)
        {
            ...
            for (size_t i = 0; i < fieldNormsChanged.length; i++)
            {
                ...
                {
                    const wchar_t* curField = _fieldInfos->fieldInfo(i)->name;
 ...
 */
const wchar_t * SegmentReader::getSegmentName()
{
...
const std::wstring SegmentReader::getClassName()
{
    return L"SegmentReader";
}

const std::wstring SegmentReader::getObjectName() const
{
    return getClassName();
}
