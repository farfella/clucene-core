...
void FieldsReader::addField(CL_NS(document)::Document& doc, const FieldInfo* fi, const bool binary, const bool compressed, const bool tokenize) {
    ...
    //no need to clean up, Field consumes b
...
#ifndef _ASCII
      //convert to utf8
      wchar_t* result = _CL_NEWARRAY(wchar_t, data.length);
      ...
      wchar_t* str = fieldsStream->readString();
...
FieldsReader::LazyField::LazyField(FieldsReader* _parent, const wchar_t* _name,
...
const wchar_t* FieldsReader::LazyField::stringValue() {
	parent->ensureOpen();
	if (fieldsData == NULL) {
		CL_NS(store)::IndexInput* localFieldsStream = getFieldStream();
		localFieldsStream->seek(pointer);
...
#ifndef _ASCII
      wchar_t* str = _CL_NEWARRAY(wchar_t, uncompressed.length);
      ...
      fieldsData = uncompressed.values;
			...
			wchar_t* chars = _CL_NEWARRAY(wchar_t, toRead+1);
	...
	return static_cast<const wchar_t*>(fieldsData); //instanceof String ? (String) fieldsData : null;
}
...
const wchar_t* FieldsReader::FieldForMerge::stringValue() const {
	return (valueType & VALUE_STRING) ? static_cast<wchar_t*>(fieldsData) : NULL;
}
