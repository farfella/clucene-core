...
IndexFileDeleter::CommitPoint::CommitPoint(IndexFileDeleter* _this, SegmentInfos* segmentInfos)
{
    ...
    for (int32_t i = 0; i < size; i++)
    {
        ...
        {
            const std::vector<std::wstring>& ff = segmentInfo->files();
...
*/
std::wstring IndexFileDeleter::CommitPoint::getSegmentsFileName()
{
...
const std::vector<std::wstring>& IndexFileDeleter::CommitPoint::getFileNames()
{
    return files;
}
...
const std::wstring IndexFileDeleter::CommitPoint::getClassName()
{
    return L"IndexFileDeleter::CommitPoint";
}
const std::wstring IndexFileDeleter::CommitPoint::getObjectName() const
{
    return getClassName();
}
...
void IndexFileDeleter::setInfoStream(std::wostream* infoStream)
{
    this->infoStream = infoStream;
    if (infoStream != NULL)
    {
        const std::wstring objectName = policy->getObjectName();

        std::wstring msg = L"setInfoStream deletionPolicy=" + objectName;
        message(msg);
    }
}

void IndexFileDeleter::message(std::wstring message)
{
    (*infoStream) << std::wstring(L"IFD [") << Misc::toString(_LUCENE_CURRTHREADID) << std::wstring(L"]: ") << message << std::wstring(L"\n");
}
...
IndexFileDeleter::IndexFileDeleter(Directory* directory, IndexDeletionPolicy* policy,
    SegmentInfos* segmentInfos, std::wostream* infoStream, DocumentsWriter* docWriter) :
    refCounts(RefCountsType(true, true)), commits(CommitsType(true))
{
    ...
    if (infoStream != NULL)
    {
        std::wstring objectName = policy->getObjectName();
        message(std::wstring(L"init: current segments file is \"") + segmentInfos->getCurrentSegmentFileName() + L"\"; deletionPolicy=" + objectName);
    }
    ...
    if (!directory->list(&files))
        _CLTHROWT(CL_ERR_IO, (std::wstring(L"cannot read directory ") + directory->toString() + L": list() returned NULL").c_str());


    for (size_t i = 0; i < files.size(); i++)
    {

        std::wstring& fileName = files.at(i);
        ...
        if (filter->accept(NULL, fileName.c_str()) && !fileName.compare(IndexFileNames::SEGMENTS_GEN) == 0)
        {
            ...
            if (wcsncmp(fileName.c_str(), IndexFileNames::SEGMENTS, wcslen(IndexFileNames::SEGMENTS)) == 0)
            {
                ...
                if (SegmentInfos::generationFromSegmentsFileName(fileName.c_str()) <= currentGen)
                {
                    ...
                    catch (CLuceneError& e)
                    {
                        ...
                        if (infoStream != NULL)
                        {
                            message(L"init: hit FileNotFoundException when loading commit \"" + fileName + L"\"; skipping this commit point32_t");
                        }
                        failed = true;
                    }
                    if (!failed)
    ...
    {
        wchar_t * fileName = it->first;
        RefCount* rc = it->second;
        if (0 == rc->count)
        {
            if (infoStream != NULL)
            {
                message(std::wstring(L"init: removing unreferenced file \"") + fileName + L"\"");
            }
            deleteFile(fileName);
        }
        it++;
...
void IndexFileDeleter::deleteCommits()
{
    ...
    if (size > 0)
    {
        ...
        for (int32_t i = 0; i < size; i++)
        {
            ...
            if (infoStream != NULL)
            {
                message(L"deleteCommits: now remove commit \"" + commit->getSegmentsFileName() + L"\"");
            }
            decRef(commit->files);
        }
        commitsToDelete.clear();
...
* that segment.
*/
void IndexFileDeleter::refresh(const wchar_t * segmentName)
{
    std::vector<std::wstring> files;
    if (!directory->list(files))
        _CLTHROWT(CL_ERR_IO, (std::wstring(L"cannot read directory ") + directory->toString() + L": list() returned NULL").c_str());
    const IndexFileNameFilter* filter = IndexFileNameFilter::getFilter();
    std::wstring segmentPrefix1;
    std::wstring segmentPrefix2;
    if (segmentName != NULL)
    {
        segmentPrefix1 = std::wstring(segmentName) + L".";
        segmentPrefix2 = std::wstring(segmentName) + L"_";
    }
    ...
    for (size_t i = 0; i < files.size(); i++)
    {
        ...
        if (filter->accept(NULL, fileName.c_str()) &&
            ((segmentName == NULL || fileName.compare(0, segmentPrefix1.length(), segmentPrefix1) == 0 || fileName.compare(0, segmentPrefix2.length(), segmentPrefix2) == 0)
                && refCounts.find((wchar_t*) fileName.c_str()) == refCounts.end() && fileName.compare(IndexFileNames::SEGMENTS_GEN) != 0))
        {

            // Unreferenced file, so remove it
            if (infoStream != NULL)
            {
                message(std::wstring(L"refresh [prefix=") + segmentName + L"]: removing newly created unreferenced file \"" + fileName + L"\"");
            }
...
void IndexFileDeleter::deletePendingFiles()
{
    if (!deletable.empty())
    {
        std::vector<std::wstring> oldDeletable;
        ...
        {
            if (infoStream != NULL)
                message(L"delete pending file " + oldDeletable[i]);
...
void IndexFileDeleter::checkpoint(SegmentInfos* segmentInfos, bool isCommit)
{
    ...
    if (infoStream != NULL)
    {
        message(std::wstring(L"now checkpoint \"") + segmentInfos->getCurrentSegmentFileName() + L"\" [" +
            Misc::toString(segmentInfos->size()) + L" segments ; isCommit = " + Misc::toString(isCommit) + L"]");
    }
    ...
    const std::vector<std::wstring>* docWriterFiles = NULL;
    if (docWriter != NULL)
    {
    ...
    if (!isCommit)
    {
        ...
        for (int32_t i = 0; i < size; i++)
        {
            ...
            {
                const std::vector<std::wstring>& files = segmentInfo->files();
...
void IndexFileDeleter::incRef(const std::vector<std::wstring>& files)
{
    int32_t size = files.size();
    for (int32_t i = 0; i < size; i++)
    {
        const std::wstring& fileName = files[i];
        RefCount* rc = getRefCount(fileName.c_str());
        if (infoStream != NULL && VERBOSE_REF_COUNTS)
        {
            message(std::wstring(L"  IncRef \"") + fileName + L"\": pre-incr count is " + Misc::toString((int32_t) rc->count));
        }
        rc->IncRef();
    }
}

void IndexFileDeleter::decRef(const std::vector<std::wstring>& files)
{
    int32_t size = files.size();
    for (int32_t i = 0; i < size; i++)
    {
...
void IndexFileDeleter::decRef(const std::wstring& fileName)
{
    RefCount* rc = getRefCount(fileName.c_str());
    if (infoStream != NULL && VERBOSE_REF_COUNTS)
    {
        std::wstring filename = fileName;
        message(std::wstring(L"  DecRef \"") + fileName + L"\": pre-decr count is " + Misc::toString((int32_t) rc->count));
    }
    if (0 == rc->DecRef())
    {
        ...
        deleteFile(fileName.c_str());
        refCounts.remove((wchar_t*) fileName.c_str());
    }
}
...
IndexFileDeleter::RefCount* IndexFileDeleter::getRefCount(const wchar_t * fileName)
{
    RefCount* rc;
    RefCountsType::iterator itr = refCounts.find((wchar_t*) fileName);
    if (itr == refCounts.end())
    {
        rc = _CLNEW RefCount();
        refCounts.put(wcsdup(fileName), rc);
    }
    else
    {
        rc = itr->second;
...
void IndexFileDeleter::deleteFiles(std::vector<std::wstring>& files)
{
    int32_t size = files.size();
    for (int32_t i = 0; i < size; i++)
...
void IndexFileDeleter::deleteNewFiles(const std::vector<std::wstring>& files)
{
    int32_t size = files.size();
    for (int32_t i = 0; i < size; i++)
        if (refCounts.find((wchar_t *) files[i].c_str()) == refCounts.end())
            deleteFile(files[i].c_str());
}

void IndexFileDeleter::deleteFile(const wchar_t * fileName)
{
    try
    {
        if (infoStream != NULL)
    ...
    {       // if delete fails
        ...
        if (directory->fileExists(fileName))
        {
            ...
            {
                std::wstring filename = fileName;
                std::wstring what = e.twhat();
                message(L"IndexFileDeleter: unable to remove file \"" + filename + L"\": " + what + L"; Will re-try later.");
            }
