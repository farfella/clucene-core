...
void DocumentsWriter::writeSegment(std::vector<std::wstring>& flushedFiles) {
  ...
  while(start < numAllFields) {
    ...
    const wchar_t* fieldName = allFields[start]->fieldInfo->name;

    int32_t end = start+1;
    while(end < numAllFields && wcscmp(allFields[end]->fieldInfo->name, fieldName)==0 )
      end++;
...
int32_t pos2=0;
  while(true) {
    const wchar_t c1 = text1[pos1++];
    const wchar_t c2 = text2[pos2++];
    if (c1 < c2)
...
void DocumentsWriter::appendPostings(ArrayBase<ThreadState::FieldData*>* fields,
  ...
  while(numFields > 0) {
    ...
    for(int32_t i=1;i<numFields;i++) {
      const wchar_t* text = mergeStates[i]->text;
    ...
    const wchar_t* start = termStates[0]->text + termStates[0]->textOffset;
...
void DocumentsWriter::addDeleteTerm(Term* term, int32_t docCount) {
  ...
  if (num == NULL) {
    ...
    numBytesUsed += ( wcslen(term->field()) + term->textLength()) * BYTES_PER_CHAR
...
wchar_t* DocumentsWriter::gewchar_tBlock() {
	SCOPED_LOCK_MUTEX(THIS_LOCK)
  ...
  if (0 == size) {
    ...
    balanceRAM();
    c = _CL_NEWARRAY(wchar_t, CHAR_BLOCK_SIZE);
    memset(c,0,sizeof(wchar_t) * CHAR_BLOCK_SIZE);
  } else{
...
void DocumentsWriter::recycleBlocks(ArrayBase<wchar_t*>& blocks, int32_t start, int32_t numBlocks) {
	SCOPED_LOCK_MUTEX(THIS_LOCK)
...
DocumentsWriter::CharBlockPool::CharBlockPool(DocumentsWriter* _parent):
    BlockPool<wchar_t>(_parent, CHAR_BLOCK_SIZE, false)
{
}
DocumentsWriter::CharBlockPool::~CharBlockPool(){
}
wchar_t* DocumentsWriter::CharBlockPool::getNewBlock(bool){
    return parent->gewchar_tBlock();
}
