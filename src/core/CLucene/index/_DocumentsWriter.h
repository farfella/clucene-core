...
private:
    ...
    std::wstring segment;                         // Current segment we are working on
    std::wstring docStoreSegment;                 // Current doc-store segment we are writing
    ...
    public:
      ...
      void close();
        ...
        const std::wstring getDirectoryType() const;
    ...
    int64_t segmentSize(const std::wstring& segmentName);
    ...
    typedef CL_NS(util)::CLArrayList<wchar_t*, CL_NS(util)::Deletor::tcArray> FreeCharBlocksType;
    ...
    std::vector<std::wstring>* _files;                      // Cached list of files we've created
    std::vector<std::wstring>* _abortedFiles;               // List of files that were written before last abort()
    ...
    void writeSegment(std::vector<std::wstring>& flushedFiles);

    /** Returns the name of the file with this extension, on
     *  the current segment we are working on. */
    std::wstring segmentFileName(const std::wstring& extension);
    std::wstring segmentFileName(const wchar_t * extension);
    ...
    private:
        ...
        wchar_t* maxTermPrefix;                 // Non-null prefix of a too-large term if this
                                              // doc has one
    ...
    class CharBlockPool : public BlockPool<wchar_t>
    {
    public:
        CharBlockPool(DocumentsWriter* _parent);
        virtual ~CharBlockPool();
        wchar_t* getNewBlock(bool trackAllocations);
        void reset();
        friend class DocumentsWriter::FieldMergeState;
    };
...
public:
     ...
     *  * is true. */
    const std::wstring& getDocStoreSegment();
    ...
    std::wstring closeDocStore();

    const std::vector<std::wstring>* abortedFiles();
    ...
    void createCompoundFile(const std::wstring& segment);
    ...
    *  called only during commit, to create the .nrm file. */
    void writeNorms(const std::wstring& segmentName, int32_t totalNumDoc);

    int32_t compareText(const wchar_t* text1, const wchar_t* text2);
    ...
    wchar_t* gewchar_tBlock();

    /* Return a char[] to the pool */
    void recycleBlocks(CL_NS(util)::ArrayBase<wchar_t*>& blocks, int32_t start, int32_t numBlocks);

    std::wstring toMB(int64_t v);
