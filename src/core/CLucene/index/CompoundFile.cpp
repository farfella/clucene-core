...
int64_t CompoundFileReader::fileLength(const char* name) const{
  ...
  if (e == NULL){
     ...
     strcpy_s(buf,"File ");
     strncat_s(buf,name,CL_MAX_PATH );
     strcat_s(buf," does not exist");
...
void CompoundFileWriter::addFile(const char* file){
  ...
  if (_internal->ids.find((char*)file)!=_internal->ids.end()){
     char buf[CL_MAX_PATH + 30];
     strcpy_s(buf,"File ");
     strncat_s(buf,file,CL_MAX_PATH);
     strcat_s(buf," already added");
     _CLTHROWA(CL_ERR_IO,buf);
  }
  _internal->ids.insert(_strdup(file));
...
void CompoundFileWriter::copyFile(WriterFileEntry* source, IndexOutput* os, uint8_t* buffer, int32_t bufferLength){
  ...
  try {
      ...
      if (remainder != 0){
         ...
         _snwprintf(buf,CL_MAX_PATH+100,L"Non-zero remainder length after copying"
      ...
      if (diff != length){
         wchar_t buf[100];
         _snwprintf(buf,100, L"Difference in the output file offsets %d "
