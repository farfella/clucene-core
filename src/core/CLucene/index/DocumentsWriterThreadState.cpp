  ...
  _parent(__parent)
{
  ...
  stringReader = _CLNEW ReusableStringReader(L"",0,false);
...
void DocumentsWriter::ThreadState::init(Document* doc, int32_t docID) {
  ...
  for(int32_t i=0;i<numDocFields;i++) {
    ...
    while(fp != NULL && wcscmp(fp->fieldInfo->name, fi->name) != 0 )
      fp = fp->next;
...
/*
  ...
  wchar_t* text = charPool->buffers[p->textStart >> CHAR_BLOCK_SHIFT];
...
bool DocumentsWriter::ThreadState::postingEquals(const wchar_t* tokenText, const int32_t tokenTextLen) {

  const wchar_t* text = charPool->buffers[p->textStart >> CHAR_BLOCK_SHIFT];
  assert (text != NULL);
...
int32_t DocumentsWriter::ThreadState::comparePostings(Posting* p1, Posting* p2) {
  const wchar_t* pos1 = charPool->buffers[p1->textStart >> CHAR_BLOCK_SHIFT] + (p1->textStart & CHAR_BLOCK_MASK);
  const wchar_t* pos2 = charPool->buffers[p2->textStart >> CHAR_BLOCK_SHIFT] + (p2->textStart & CHAR_BLOCK_MASK);
  while(true) {
    const wchar_t c1 = *pos1++;
    const wchar_t c2 = *pos2++;
    if (c1 < c2)
...
bool DocumentsWriter::ThreadState::FieldData::sort(FieldData* e1, FieldData* e2){
  return wcscmp(e1->fieldInfo->name, e2->fieldInfo->name) < 0;
}
...
int32_t DocumentsWriter::ThreadState::FieldData::compareTo(NamedObject* o) {
  if ( o->getObjectName() != FieldData::getClassName() )
    return -1;
  return wcscmp(fieldInfo->name, ((FieldData*) o)->fieldInfo->name);
}
...
void DocumentsWriter::ThreadState::FieldData::invertField(Field* field, Analyzer* analyzer, const int32_t maxFieldLength) {
  ...
  if (!field->isTokenized()) {     // un-tokenized field
    const wchar_t* stringValue = field->stringValue();
    const size_t valueLength = wcslen(stringValue);
    ...
    if (streamValue != NULL)
      ...
      if (readerValue != NULL)
        ...
        const wchar_t* stringValue = field->stringValue();
        size_t stringValueLength = wcslen(stringValue);
        if (stringValue == NULL)
        ...
        ++length;
	          ...
	          if (_parent->infoStream != NULL)
	            (*_parent->infoStream) << "maxFieldLength "  << maxFieldLength << " reached for field " << fieldInfo->name << ", ignoring following tokens\n";
					...
					const wchar_t* errMsgBase =
						L"Indexing a huge number of tokens from a single"
						L" field (\"%s\", in this case) can cause CLucene"
						L" to use memory excessively."
						L"  By default, CLucene will accept only %s tokens"
						L" tokens from a single field before forcing the"
						L" client programmer to specify a threshold at"
						L" which to truncate the token stream."
						L"  You should set this threshold via"
						L" IndexReader::maxFieldLength (set to LUCENE_INT32_MAX"
						L" to disable truncation, or a value to specify maximum number of fields).";

					wchar_t defaultMaxAsChar[34];
					_i64tow(IndexWriter::DEFAULT_MAX_FIELD_LENGTH,
						defaultMaxAsChar, 10
					);
					int32_t errMsgLen = wcslen(errMsgBase)
						+ wcslen(fieldInfo->name)
						+ wcslen(defaultMaxAsChar);
					wchar_t* errMsg = _CL_NEWARRAY(wchar_t,errMsgLen+1);

					_snwprintf(errMsg, errMsgLen,errMsgBase, fieldInfo->name, defaultMaxAsChar);
...
void DocumentsWriter::ThreadState::FieldData::addPosition(Token* token) {
  ...
  // provide a String token or offset into a wchar_t*
  // array
  const wchar_t* tokenText = token->termBuffer();
...
*/
      ...
      if (textLen1 + threadState->charPool->tUpto > CHAR_BLOCK_SIZE) {
        if (textLen1 > CHAR_BLOCK_SIZE) {
          ...
          if (threadState->maxTermPrefix == NULL){
            threadState->maxTermPrefix = _CL_NEWARRAY(wchar_t,31);
            wcsncpy(threadState->maxTermPrefix,tokenText,30);
      ...
      wchar_t* text = threadState->charPool->buffer;
      wchar_t* textUpto = text+ threadState->charPool->tUpto;
      ...
      wcsncpy(textUpto, tokenText, tokenTextLen);
...
void DocumentsWriter::ThreadState::FieldData::writeVectors(FieldInfo* fieldInfo) {
  ...
  for(int32_t j=0;j<numPostingsVectors;j++) {
    ...
    int32_t prefix = 0;
    const wchar_t* text2 = threadState->charPool->buffers[posting->textStart >> CHAR_BLOCK_SHIFT];
    const wchar_t* start2 = text2 + (posting->textStart & CHAR_BLOCK_MASK);
    const wchar_t* pos2 = start2;
    ...
    if (lastPosting == NULL)
      ...
      const wchar_t* text1 = threadState->charPool->buffers[lastPosting->textStart >> CHAR_BLOCK_SHIFT];
      const wchar_t* start1 = text1 + (lastPosting->textStart & CHAR_BLOCK_MASK);
      const wchar_t* pos1 = start1;
      while(true) {
