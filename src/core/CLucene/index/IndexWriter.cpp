...
const wchar_t * IndexWriter::WRITE_LOCK_NAME = L"write.lock";
std::wostream* IndexWriter::defaultInfoStream = NULL;
...
void IndexWriter::message(std::wstring message)
{
    if (infoStream != NULL)
    {
        (*infoStream) << std::wstring(L"IW ") << Misc::toString(messageID) << std::wstring(L" [")
            << Misc::toString(_LUCENE_CURRTHREADID) << std::wstring(L"]: ") << message << std::wstring(L"\n");
    }
}
...
IndexWriter::IndexWriter(const wchar_t * path, Analyzer* a, bool create) :bOwnsDirectory(true)
{
    ...
    IndexDeletionPolicy* deletionPolicy, const bool autoCommit)
{
    ...
    try
    {
        ...
        if (!hasLock)
        {// obtain write lock
            std::string msg = "Index locked for write: " + lucene_wcstoutf8string(writeLock->getObjectName().c_str(), -1);
            _CLTHROWA(CL_ERR_LockObtainFailed, msg.c_str());
        }
    }
    catch (...)
    {
    ...
    try
    {
        ...
        if (infoStream != NULL)
        {
            message(std::wstring(L"init: create=") + (create ? L"true" : L"false"));
            messageState();
        }
...
void IndexWriter::setMergePolicy(MergePolicy* mp)
{
    ...
    if (infoStream != NULL)
    {
        std::wstring objectName = mp->getObjectName();
        message(std::wstring(L"setMergePolicy ") + objectName);
    }
}
...
void IndexWriter::setMergeScheduler(MergeScheduler* mergeScheduler)
{
    ...
    if (infoStream != NULL)
    {
        const std::wstring objectName = mergeScheduler->getObjectName();
        message(L"setMergeScheduler " + objectName);
    }
}
...
void IndexWriter::setMaxFieldLength(int32_t maxFieldLength)
{
    ...
    if (infoStream != NULL)
        message(L"setMaxFieldLength " + Misc::toString(maxFieldLength));
}
...
void IndexWriter::setMaxBufferedDocs(int32_t maxBufferedDocs)
{
    ...
    pushMaxBufferedDocs();
    if (infoStream != NULL)
        message(L"setMaxBufferedDocs " + Misc::toString(maxBufferedDocs));
}
    ...
    if (docWriter->getMaxBufferedDocs() != DISABLE_AUTO_FLUSH)
    {
        ...
        if (mp->instanceOf(LogDocMergePolicy::getClassName()))
        {
            ...
            if (lmp->getMinMergeDocs() != maxBufferedDocs)
            {
                if (infoStream != NULL)
                {
                    message(std::wstring(L"now push maxBufferedDocs ") + Misc::toString(maxBufferedDocs) + L" to LogDocMergePolicy");
                }
...
void IndexWriter::setRAMBufferSizeMB(float_t mb)
{
    ...
    if (infoStream != NULL)
    {
        message(std::wstring(L"setRAMBufferSizeMB ") + Misc::toString(mb));
    }
}
...
void IndexWriter::setMaxBufferedDeleteTerms(int32_t maxBufferedDeleteTerms)
{
    ...
    if (infoStream != NULL)
        message(L"setMaxBufferedDeleteTerms " + Misc::toString(maxBufferedDeleteTerms));
}
...
void IndexWriter::setDefaultInfoStream(std::wostream* infoStream)
{
    IndexWriter::defaultInfoStream = infoStream;
}

std::wostream* IndexWriter::getDefaultInfoStream()
{
    return IndexWriter::defaultInfoStream;
}

//TODO: infoStream - unicode
void IndexWriter::setInfoStream(std::wostream* infoStream)
{
    ensureOpen();
...
{
    message(std::wstring(L"setInfoStream: dir=") + directory->toString() +
        L" autoCommit=" + (autoCommit ? L"true" : L"false" +
            std::wstring(L" mergePolicy=") + mergePolicy->getObjectName() +
            L" mergeScheduler=" + mergeScheduler->getObjectName() +
            L" ramBufferSizeMB=" + Misc::toString(docWriter->getRAMBufferSizeMB()) +
            L" maxBuffereDocs=" + Misc::toString(docWriter->getMaxBufferedDocs())) +
        L" maxBuffereDeleteTerms=" + Misc::toString(docWriter->getMaxBufferedDeleteTerms()) +
        L" maxFieldLength=" + Misc::toString(maxFieldLength) +
        L" index=" + segString());
}

std::wostream* IndexWriter::getInfoStream()
{
    ensureOpen();
    return infoStream;
}
...
void IndexWriter::closeInternal(bool waitForMerges)
{
    ...
    {
        if (infoStream != NULL)
            message(std::wstring(L"now flush at close"));
            ...
            SCOPED_LOCK_MUTEX(this->THIS_LOCK)
                if (commitPending)
                {
                    ...
                    try
                    {
                        ...
                        {
                            if (infoStream != NULL)
                                message(std::wstring(L"hit exception committing segments file during close"));
                            deletePartialSegmentsFile();
                        }
                    )
                        if (infoStream != NULL)
                            message(std::wstring(L"close: wrote segments file \"") + segmentInfos->getCurrentSegmentFileName() + L"\"");
...
closing = false;
if (infoStream != NULL)
  message(std::wstring(L"hit exception while closing"));
}
...
bool IndexWriter::flushDocStores()
{
    SCOPED_LOCK_MUTEX(THIS_LOCK)
        ...
        const std::vector<std::wstring>& files = docWriter->files();

    bool useCompoundDocStore = false;

    if (files.size() > 0)
    {
        std::wstring docStoreSegment;
        ...
        try
        {
            ...
            {
                if (infoStream != NULL)
                    message(std::wstring(L"hit exception closing doc store segment"));
        ...
        if (useCompoundDocStore && !docStoreSegment.empty())
        {
            ...
            const std::wstring compoundFileName = docStoreSegment + L"." + IndexFileNames::COMPOUND_FILE_STORE_EXTENSION;
    ...
    try
    {
        try
        {
            ...
            if (!success)
            {
                ...
                if (infoStream != NULL)
                    message(std::wstring(L"hit exception adding document"));

                {
                    SCOPED_LOCK_MUTEX(this->THIS_LOCK)
                        ...
                        if (docWriter != NULL)
                        {
                            const std::vector<std::wstring>* files = docWriter->abortedFiles();
                            if (files != NULL)
    ...
    try
    {
        ...
        try
        {
            ...
            if (!success)
            {
                ...
                if (infoStream != NULL)
                    message(std::wstring(L"hit exception updating document"));
...
std::wstring IndexWriter::newSegmentName()
{
                        ...
                        for (int32_t i = 0; i < size; i++)
                        {
                            ...
                            if (_merge->optimize)
                            {
                                ...
                                CLuceneError err(tmp.number(),
                                    (std::wstring(L"background merge hit exception: ") + _merge->segString(directory) + L":" + tmp.twhat()).c_str(), false);
                                throw err;
                            }
                        }
                    }
...
void IndexWriter::startTransaction()
{
    ...
    CND_PRECONDITION(docWriter->getNumBufferedDeleteTerms() == 0,
        L"calling startTransaction with buffered delete terms not supported");
    CND_PRECONDITION(docWriter->getNumDocsInRAM() == 0,
        L"calling startTransaction with buffered documents not supported");
    ...
    if (localAutoCommit)
    {
        ...
        if (infoStream != NULL)
            message(L"flush at startTransaction");
...
void IndexWriter::commitTransaction()
{
    ...
    try
    {
        ...
        {
            if (infoStream != NULL)
                message(L"hit exception committing transaction");
    ...
    SCOPED_LOCK_MUTEX(THIS_LOCK)
        if (!waitForMerges)
        {
                ...
                it != pendingMerges->end(); it++)
            {
                MergePolicy::OneMerge* _merge = *it;
                if (infoStream != NULL)
                    message(L"now abort pending merge " + _merge->segString(directory));
                _merge->abort();
                mergeFinish(_merge);
                ...
                it != runningMerges->end(); it++)
            {
                MergePolicy::OneMerge* _merge = *it;
                if (infoStream != NULL)
                    message(L"now abort running merge " + _merge->segString(directory));
                _merge->abort();
            ...
            {
                if (infoStream != NULL)
                    message(L"now wait for " + Misc::toString((int32_t) runningMerges->size()) + L" running merge to abort");
                CONDITION_WAIT(THIS_LOCK, THIS_WAIT_CONDITION)
            }
...
void IndexWriter::checkpoint()
{
    SCOPED_LOCK_MUTEX(THIS_LOCK)
        if (autoCommit)
        {
            ...
            commitPending = false;
            if (infoStream != NULL)
                message(L"checkpoint: wrote segments file \"" + segmentInfos->getCurrentSegmentFileName() + L"\"");
        }
        else
        {
            commitPending = true;
    ...
    while (true)
    {
        ...
        if (_merge != NULL)
        {
            if (registerMerge(_merge))
            {
                ...
                _CLTHROWT(CL_ERR_Merge, (std::wstring(L"segment \"") + info->name + L" exists in external directory yet the MergeScheduler executed the merge in a separate thread").c_str());
        }
        else
...
bool IndexWriter::doFlush(bool _flushDocStores)
{
    ...
    try
    {
        ...
        std::wstring docStoreSegment = docWriter->getDocStoreSegment();
        ...
        if (infoStream != NULL)
        {
            message(L"  flush: segment=" + docWriter->getSegment() +
                L" docStoreSegment=" + docWriter->getDocStoreSegment() +
                L" docStoreOffset=" + Misc::toString(docWriter->getDocStoreOffset()) +
                L" flushDocs=" + Misc::toString(flushDocs) +
                L" flushDeletes=" + Misc::toString(flushDeletes) +
                L" flushDocStores=" + Misc::toString(_flushDocStores) +
                L" numDocs=" + Misc::toString(numDocs) +
                L" numBufDelTerms=" + Misc::toString(docWriter->getNumBufferedDeleteTerms()));
        ...
        std::wstring segment = docWriter->getSegment();
            ...
            try
            {
                ...
                if (!success)
                {
                    ...
                    if (flushDeletes)
                    {
                        ...
                        for (int32_t i = 0; i < size; i++)
                        {
                            const std::wstring newDelFileName = segmentInfos->info(i)->getDelFileName();
                            const std::wstring delFileName = rollback->info(i)->getDelFileName();
                ...
                newSegment))
            {
                ...
                try
                {
                    ...
                    if (!success)
                    {
                        ...
                        deleter->deleteFile((segment + L"." + IndexFileNames::COMPOUND_FILE_EXTENSION).c_str());
                        deletePartialSegmentsFile();
                    }
                )
...
int32_t IndexWriter::ensureContiguousMerge(MergePolicy::OneMerge* _merge)
{
    ...
    if (first == -1)
        _CLTHROWA(CL_ERR_Merge, (std::wstring(L"could not find segment ") + _merge->segments->info(0)->name + L" in current segments").c_str());
    ...
    for (int32_t i = 0; i < numSegmentsToMerge; i++)
    {
        ...
        if (first + i >= numSegments || !segmentInfos->info(first + i)->equals(info))
        {
            ...
            {
                std::wstring msg = L"MergePolicy selected a segment (" + info->name + L") that is not in the index";
                _CLTHROWA(CL_ERR_Merge, msg.c_str());
            }
            else
            {
                std::wstring objectName = _merge->getObjectName();
                std::wstring msg = L"MergePolicy selected non-contiguous segments to merge (" + objectName + L" vs " + segString() + L"), which IndexWriter (currently) cannot handle";
                _CLTHROWA(CL_ERR_Merge, msg.c_str());
            }
        }
    }
...
bool IndexWriter::commitMerge(MergePolicy::OneMerge* _merge)
{
    ...
    if (infoStream != NULL)
        message(L"commitMerge: " + _merge->segString(directory));
    ...
    {
        if (infoStream != NULL)
            message(L"commitMerge: skipping merge " + _merge->segString(directory) + L": it was aborted");
    ...
    try
    {
        ...
        {
            if (infoStream != NULL)
                message(std::wstring(L"hit exception creating merged deletes file"));
            deleter->refresh(_merge->info->name.c_str());
        }
    )
        ...
        const std::wstring mergeDocStoreSegment = _merge->info->getDocStoreSegment();
    if (!mergeDocStoreSegment.empty() && !_merge->info->getDocStoreIsCompoundFile())
    {
        ...
        for (int32_t i = 0; i < size; i++)
        {
            ...
            const std::wstring docStoreSegment = info->getDocStoreSegment();
            if (!docStoreSegment.empty() &&
    ...
    try
    {
        ...
        {
            if (infoStream != NULL)
                message(std::wstring(L"hit exception when checkpointing after merge"));
            segmentInfos->clear();
    ...
    try
    {
        try
        {
            try
            {
                ...
                if (infoStream != NULL)
                    message(L"now merge\n  merge=" + _merge->segString(directory) + L"\n  index=" + segString());
...
{
if (infoStream != NULL)
  message(std::wstring(L"hit exception during merge"));
addMergeException(_merge);
...
bool IndexWriter::registerMerge(MergePolicy::OneMerge* _merge)
{
    ...
    if (infoStream != NULL)
        message(std::wstring(L"add merge to pendingMerges: ") + _merge->segString(directory) + L" [total " + Misc::toString((int32_t) pendingMerges->size()) + L" pending]");
...
void IndexWriter::_mergeInit(MergePolicy::OneMerge* _merge)
{
    ...
    std::wstring lastDocStoreSegment;
    int32_t next = -1;

    bool mergeDocStores = false;
    bool doFlushDocStore = false;
    const std::wstring currentDocStoreSegment = docWriter->getDocStoreSegment();
    ...
    for (int32_t i = 0; i < end; i++)
    {
        ...
        std::wstring docStoreSegment = si->getDocStoreSegment();
        if (docStoreSegment.empty())
            mergeDocStores = true;
    ...
    if (mergeDocStores && doFlushDocStore)
    {
        ...
        if (infoStream != NULL)
            message(std::wstring(L"flush at merge"));
        flush(false, true);
    }
...
int32_t IndexWriter::mergeMiddle(MergePolicy::OneMerge* _merge)
{
    ...
    const std::wstring mergedName = _merge->info->name;
    ...
    if (infoStream != NULL)
        message(L"merging " + _merge->segString(directory));
    ...
    try
    {
        ...
        if (infoStream != NULL)
        {
            message(std::wstring(L"merge: total ") + Misc::toString(totDocCount) + L" docs");
        }
    ...
    {
        if (infoStream != NULL)
            message(L"hit exception during merge; now refresh deleter on segment " + mergedName);
        {
            SCOPED_LOCK_MUTEX(this->THIS_LOCK)
                addMergeException(_merge);
    ...
    if (_merge->useCompoundFile)
    {
        ...
        success = false;
        bool skip = false;
        const std::wstring compoundFileName = mergedName + L"." + IndexFileNames::COMPOUND_FILE_EXTENSION;
        ...
        try
        {
...

                {
                    SCOPED_LOCK_MUTEX(this->THIS_LOCK)
                        if (segmentInfos->indexOf(_merge->info) == -1)
                        {
                            ...
                            // file creating.
                            if (infoStream != NULL)
                                message(L"hit exception creating compound file; ignoring it because our info (segment " + _merge->info->name + L") has been merged away");
                            skip = true;
                        }
                        else
                            throw ioe;
            ...
            {
                if (infoStream != NULL)
                    message(std::wstring(L"hit exception creating compound file during merge: skip=") + Misc::toString(skip));
                    ...
                    SCOPED_LOCK_MUTEX(this->THIS_LOCK)
                        ...
                        else
                        {
                            ...
                            try
                            {
                                ...
                                {
                                    if (infoStream != NULL)
                                        message(std::wstring(L"hit exception checkpointing compound file during merge"));
...
void IndexWriter::deletePartialSegmentsFile()
{
    ...
    {
        std::wstring segmentFileName = IndexFileNames::fileNameFromGeneration(IndexFileNames::SEGMENTS,
            L"",
            segmentInfos->getGeneration());
        if (infoStream != NULL)
            message(L"now delete partial segments file \"" + segmentFileName + L"\"");
...
void IndexWriter::applyDeletes(bool flushedNewSegment)
{
    ...
    if (infoStream != NULL)
        message(std::wstring(L"flush ") + Misc::toString(docWriter->getNumBufferedDeleteTerms()) +
            L" buffered deleted terms and " + Misc::toString((int32_t) bufferedDeleteDocIDs->size()) +
            L" deleted docIDs on " + Misc::toString((int32_t) segmentInfos->size()) + L" segments.");
...
std::wstring IndexWriter::segString()
{
    SCOPED_LOCK_MUTEX(THIS_LOCK)
        std::wstring buffer;
    for (int32_t i = 0; i < segmentInfos->size(); i++)
