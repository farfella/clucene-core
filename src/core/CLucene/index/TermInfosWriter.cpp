...
CL_NS_DEF(index)
  ...
  void TermInfosWriter::add(Term* term, TermInfo* ti){
    ...
    wcsncpy(termTextBuffer.values, term->text(), length);

    add(fieldInfos->fieldNumber(term->field()), termTextBuffer.values, length, ti);
  }

  // Currently used only by assert statement
  int32_t TermInfosWriter::compareToLastTerm(int32_t fieldNumber, const wchar_t* termText, int32_t length) {
    int32_t pos = 0;

    if (lastFieldNumber != fieldNumber) {
      const int32_t cmp = wcscmp(fieldInfos->fieldName(lastFieldNumber), fieldInfos->fieldName(fieldNumber));
    ...
    while(pos < length && pos < lastTermTextLength) {
      const wchar_t c1 = lastTermText[pos];
      const wchar_t c2 = termText[pos];
      if (c1 < c2)
        return -1;
      else if (c1 > c2)
        return 1;
	...
	void TermInfosWriter::add(int32_t fieldNumber, const wchar_t* termText, int32_t termTextLength, const TermInfo* ti) {
...
// TODO: This is a hack. If _ASCII is defined, Misc::toString(const wchar_t*, int) will cause linking errors,
//       at least on VS. Needs a prettier fix no doubt... ISH 2009-11-08
    ...
    if ( termText != NULL )
      wcsncpy(lastTermText.values,termText,termTextLength);
    else
  ...
  void TermInfosWriter::writeTerm(int32_t fieldNumber, const wchar_t* termText, int32_t termTextLength){
