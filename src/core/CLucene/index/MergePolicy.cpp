...
const std::wstring MergePolicy::OneMerge::getClassName()
{
    return L"MergePolicy::OneMerge";
}
const std::wstring MergePolicy::OneMerge::getObjectName() const
{
    return getClassName();
}
...
void MergePolicy::OneMerge::checkAborted(CL_NS(store)::Directory* dir)
{
    SCOPED_LOCK_MUTEX(THIS_LOCK)
        if (aborted)
            _CLTHROWT(CL_ERR_MergeAborted, (std::wstring(L"merge is aborted: ") + segString(dir)).c_str());
}

std::wstring MergePolicy::OneMerge::segString(CL_NS(store)::Directory* dir) const
{
    std::wstring b;
    ...
    if (info != NULL)
        b.append(L" into ").append(info->name);
    if (optimize)
        b.append(L" [optimize]");
    return b;
}
...
std::wstring MergePolicy::MergeSpecification::segString(CL_NS(store)::Directory* dir)
{
    std::wstring b = L"MergeSpec:\n";
...
void LogMergePolicy::message(const std::wstring& message)
{
    if (writer != NULL)
    {
        std::wstring msg = L"LMP: " + message;
        writer->message(msg);
    }
}
...
MergePolicy::MergeSpecification* LogMergePolicy::findMerges(SegmentInfos* infos, IndexWriter* writer)
{
    ...
    const int32_t numSegments = infos->size();
    this->writer = writer;
    MESSAGE(std::wstring(L"findMerges: ") + Misc::toString(numSegments) + L" segments");
    ...
    while (start < numSegments)
    {
        ...
        MESSAGE(std::wstring(L"  level ") + Misc::toString(levelBottom) + L" to " + Misc::toString(maxLevel) + L": " + Misc::toString(1 + upto - start) + L" segments");
        ...
        while (end <= 1 + upto)
        {
            ...
            if (!anyTooLarge)
            {
                if (spec == NULL)
                    spec = _CLNEW MergeSpecification();
                MESSAGE(std::wstring(L"    ") + Misc::toString(start) + L" to " + Misc::toString(end) + L": add this merge");
                SegmentInfos* range = _CLNEW SegmentInfos;
                infos->range(start, end, *range);
                spec->add(_CLNEW OneMerge(range, _useCompoundFile));
            }
            else
                MESSAGE(std::wstring(L"    ") + Misc::toString(start) + L" to " + Misc::toString(end) + L": contains segment over maxMergeSize or maxMergeDocs; skipping");
...
const std::wstring LogMergePolicy::getClassName()
{
    return L"LogMergePolicy";
}
const std::wstring LogMergePolicy::getObjectName() const
{
    return getClassName();
}

bool LogMergePolicy::instanceOf(const std::wstring& other) const
{
    std::wstring t = this->getObjectName();
...
const std::wstring LogDocMergePolicy::getClassName()
{
    return L"LogDocMergePolicy";
}
const std::wstring LogDocMergePolicy::getObjectName() const
{
    return getClassName();
}
...
const std::wstring LogByteSizeMergePolicy::getClassName()
{
    return L"LogByteSizeMergePolicy";
}
const std::wstring LogByteSizeMergePolicy::getObjectName() const
{
    return getClassName();
}
