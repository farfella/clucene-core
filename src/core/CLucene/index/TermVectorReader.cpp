...
void TermVectorsReader::get(const int32_t docNum, const wchar_t* field, TermVectorMapper* mapper){
	if (tvx != NULL) {
...
TermFreqVector* TermVectorsReader::get(const int32_t docNum, const wchar_t* field){
...
ArrayBase<TermFreqVector*>* TermVectorsReader::get(const int32_t docNum){
    ...
    if (tvx != NULL) {
        ...
        if (fieldCount != 0) {
            int32_t number = 0;
            const wchar_t** fields = _CL_NEWARRAY(const wchar_t*,fieldCount+1);
...
void TermVectorsReader::readTermVectors(const wchar_t** fields, const int64_t* tvfPointers,
...
void TermVectorsReader::readTermVector(const wchar_t* field, const int64_t tvfPointer, TermVectorMapper* mapper){
  ...
  ValueArray<wchar_t> buffer(10); // init the buffer with a length of 10 character
...
void ParallelArrayTermVectorMapper::setExpectations(const wchar_t* _field, const int32_t numTerms,
													const bool storeOffsets, const bool storePositions) {
	_CLDELETE_LCARRAY(field);
	this->field = _wcsdup(_field);

  terms = _CLNEW CL_NS(util)::wchar_tArray(numTerms);
...
void ParallelArrayTermVectorMapper::map(const wchar_t* term, int32_t termLen, const int32_t frequency,
    ArrayBase<TermVectorOffsetInfo*>* _offsets,
    ArrayBase<int32_t>* _positions) {
	terms->values[currentPosition] = wcsdup(term);
