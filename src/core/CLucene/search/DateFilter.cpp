...
CL_NS_DEF(search)
  ...
  DateFilter::DateFilter(const wchar_t* f, int64_t from, int64_t to)
  {
    wchar_t* tmp = DateField::timeToString(from);
  ...
  /** Constructs a filter for field <code>f</code> matching times before
    <code>time</code>. */
  DateFilter* DateFilter::Before(const wchar_t* field, int64_t time) {
    return _CLNEW DateFilter(field, 0,time);
  }

  /** Constructs a filter for field <code>f</code> matching times after
    <code>time</code>. */
  DateFilter* DateFilter::After(const wchar_t* field, int64_t time) {
