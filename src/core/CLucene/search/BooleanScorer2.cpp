...
public:
	...
	virtual wchar_t* toString() { return _wcsdup(L"NonMatchingScorer"); }

	Explanation* explain( int32_t /*doc*/ ) {
		Explanation* e = _CLNEW Explanation();
		e->setDescription(L"No document matches.");
		return e;
	}
...
public:
	...
	*/
	...
	virtual wchar_t* toString() {
		return _wcsdup(L"ReqOptSumScorer");
	}
	...
	*/
	Explanation* explain( int32_t doc ) {
		...
		res->setDescription(L"required, optional");
...
public:
	...
	*/
	...
	virtual wchar_t* toString() {
		return _wcsdup(L"ReqExclScorer");
	}

	Explanation* explain( int32_t doc ) {
		Explanation* res = _CLNEW Explanation();
		if (exclScorer->skipTo(doc) && (exclScorer->doc() == doc)) {
			res->setDescription(L"excluded");
		} else {
			res->setDescription(L"not excluded");
			res->addDetail(reqScorer->explain(doc));
		}
		return res;
	}
...
public:
	...
	virtual wchar_t* toString() {return _wcsdup(L"BSConjunctionScorer");}
};
...
public:
	...
	virtual wchar_t* toString() {return _wcsdup(L"BSDisjunctionSumScorer");}
};
...
wchar_t* BooleanScorer2::toString()
{
	return _wcsdup(L"BooleanScorer2");
}
