...
private:
	...
	wchar_t description[LUCENE_SEARCH_EXPLANATION_DESC_LEN];					// what it represents
	CL_NS(util)::CLArrayList<Explanation*,
		CL_NS(util)::Deletor::Object<Explanation> >* details;					// sub-explanations

public:
	Explanation();
	Explanation(float_t _value, const wchar_t* _description);
	virtual ~Explanation();
...
public:
	...
	ComplexExplanation(const bool _match, const float_t _value, const wchar_t* _description);
