...
SortField::SortField(const wchar_t* field)
{
    this->type = AUTO;
    this->reverse = false;
...
SortField::SortField(const wchar_t* field, int32_t type, bool reverse)
{
...
const wchar_t* SortField::getField() const
{
    return field;
}
 ...
 */
 /*SortField::SortField (wchar_t* field, Locale* locale) {
   this->field = (field != NULL) ? CLStringIntern::intern(field): field;
   this->type = STRING;
   this->locale = locale;
 }*/
  ...
  * @param locale Locale of values in the field.
  */
  /*SortField::SortField (wchar_t* field, Locale* locale, bool reverse) {
     this->field = (field != NULL) ? CLStringIntern::intern(field): field;
    this->type = STRING;
    this->locale = locale;
    this->reverse = reverse;
...
SortField::SortField(const wchar_t* field, SortComparatorSource* comparator, bool reverse)
{
    this->field = (field != NULL) ? CLStringIntern::intern(field) : field;
    this->type = CUSTOM;
    this->reverse = reverse;
...
std::wstring SortField::toString() const
{
    std::wstring buffer;
    switch (type)
    {
...
Sort::Sort(const wchar_t* field, bool reverse)
{
    this->fields = NULL;
    setSort(field, reverse);
}

Sort::Sort(const wchar_t** fields)
{
    this->fields = NULL;
    setSort(fields);
}
...
void Sort::setSort(const wchar_t* field, bool reverse)
{
    clear();
...
void Sort::setSort(const wchar_t** fieldnames)
{
    clear();
...
std::wstring Sort::toString() const
{
    std::wstring buffer;
    ...
    while (fields[i] != NULL)
    {
        ...
        std::wstring p = fields[i]->toString();
        buffer.append(p);
        i++;
    }
...
public:
    ...
    {
        CND_PRECONDITION(i->doc >= 0 && i->doc < cachedValuesLen, L"i->doc out of range")
            CND_PRECONDITION(j->doc >= 0 && j->doc < cachedValuesLen, L"j->doc out of range")
...
ScoreDocComparator* SortComparator::newComparator(CL_NS(index)::IndexReader* reader, const wchar_t* fieldname)
{
