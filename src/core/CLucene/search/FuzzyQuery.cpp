    ...
    minimumSimilarity(minSimilarity)
{
    ...
    text = _wcsdup(searchTerm->text() + realPrefixLength);
    textLen = fullSearchTermLength - realPrefixLength;

    prefix = _CL_NEWARRAY(wchar_t, realPrefixLength + 1);
    wcsncpy(prefix, searchTerm->text(), realPrefixLength);
    ...
    prefix = _CL_NEWARRAY(wchar_t,prefixLength+1);
    wcsncpy(prefix,text,prefixLength);
...
const std::wstring FuzzyTermEnum::getObjectName() const { return getClassName(); }
const std::wstring FuzzyTermEnum::getClassName() { return L"FuzzyTermEnum"; }
...
bool FuzzyTermEnum::termCompare(Term* term)
{
    ...
    if (searchTerm->field() == term->field() &&
        (prefixLength == 0 || wcsncmp(termText, prefix, prefixLength) == 0))
    {

        const wchar_t* target = termText + prefixLength;
...
float_t FuzzyTermEnum::similarity(const wchar_t* target, const size_t m)
{
    ...
    wchar_t s_i; // ith character of s
    for (i = 1; i <= n; i++)
    {
...
std::wstring FuzzyQuery::toString(const wchar_t* field) const
{
    std::wstring buffer; // TODO: Have a better estimation for the initial buffer length
    ...
    buffer.push_back(L'~');
    buffer.append(float_to_wstring(minimumSimilarity, 1));
    buffer.append(boost_to_wstring(getBoost()));
    return buffer;
}

const std::wstring FuzzyQuery::getObjectName() const
{
...
const std::wstring FuzzyQuery::getClassName()
{
    //Func - Returns the name of the query
    //Pre  - true
    //post - The string FuzzyQuery has been returned
