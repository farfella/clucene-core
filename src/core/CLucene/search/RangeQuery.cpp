...
const std::wstring RangeQuery::getObjectName() const
{
    return getClassName();
}
const std::wstring RangeQuery::getClassName()
{
    return L"RangeQuery";
}
...
Query* RangeQuery::rewrite(IndexReader* reader)
{
    ...
    try
    {
        ...
        const wchar_t* testField = getField();
        do
        {
            lastTerm = enumerator->term();
            if (lastTerm != NULL && lastTerm->field() == testField)
            {
                if (!checkLower || wcscmp(lastTerm->text(), lowerTerm->text()) > 0)
                {
                    checkLower = false;
                    if (upperTerm != NULL)
                    {
                        int compare = wcscmp(upperTerm->text(), lastTerm->text());
...
std::wstring RangeQuery::toString(const wchar_t* field) const
{
    std::wstring buffer;
    if (field == NULL || wcscmp(getField(), field) != 0)
    {
        buffer.append(getField());
        buffer.append(L":");
    }
    buffer.append(inclusive ? L"[" : L"{");
    buffer.append(lowerTerm != NULL ? lowerTerm->text() : L"NULL");
    buffer.append(L" TO ");
    buffer.append(upperTerm != NULL ? upperTerm->text() : L"NULL");
    buffer.append(inclusive ? L"]" : L"}");
    if (getBoost() != 1.0f)
    {
        buffer.append(L"^");
...
const wchar_t* RangeQuery::getField() const
{
