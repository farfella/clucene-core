...
CL_NS_DEF(search)
  ...
  wchar_t* PhraseQuery::toString(const wchar_t* f) const{
...
void PhraseQuery::extractTerms( TermSet * termset ) const
{
 ...
 wchar_t* PhraseWeight::toString() {
	return wcsdup(L"weight(PhraseQuery)");
 }
 ...
 void PhraseWeight::normalize(float_t queryNorm) {
  ...
  Explanation* PhraseWeight::explain(IndexReader* reader, int32_t doc){
	  ...
	  _snwprintf(descbuf,LUCENE_SEARCH_EXPLANATION_DESC_LEN,
		  L"idf(%s: %s)",parentQuery->field,docFreqs.getBuffer());
	  ...
	  tmp = getQuery()->toString();
	  _snwprintf(descbuf,LUCENE_SEARCH_EXPLANATION_DESC_LEN,
		  L"queryWeight(%s), product of:",tmp);
	  _CLDELETE_LCARRAY(tmp);
