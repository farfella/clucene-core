...
public:
    ...
    std::wstring toString(wchar_t* f);
...
const wchar_t* PhraseQuery::getFieldName() const { return field; }
...
const std::wstring PhraseQuery::getClassName()
{
    return L"PhraseQuery";
}
const std::wstring PhraseQuery::getObjectName() const
{
...
void PhraseQuery::add(Term* term, int32_t position)
{
    ...
    else
    {
        ...
        {
            wchar_t buf[200];
            _snwprintf(buf, 200, L"All phrase terms must be in the same field: %s", term->field());
...
Term** PhraseQuery::getTerms() const
{
    ...
    CND_CONDITION(ret != NULL, L"Could not allocated memory for ret");
...
std::wstring PhraseQuery::toString(const wchar_t* f) const
{
    ...
    {
        buffer.push_back(L'~');
        buffer.append(std::to_wstring(slop) + L".0");
    }
...
std::wstring PhraseWeight::toString()
{
    return L"weight(PhraseQuery)";
}
...
Scorer* PhraseWeight::scorer(IndexReader* reader)
{
    ...
    CND_CONDITION(tps != NULL, L"Could not allocate memory for tps");
    ...
    CND_CONDITION(ret != NULL, L"Could not allocate memory for ret");
...
Explanation* PhraseWeight::explain(IndexReader* reader, int32_t doc)
{
    ...
    wchar_t descbuf[LUCENE_SEARCH_EXPLANATION_DESC_LEN + 1];
    std::wstring tmp;

    tmp = getQuery()->toString();
    _snwprintf(descbuf, LUCENE_SEARCH_EXPLANATION_DESC_LEN, L"weight(%s in %d), product of:",
        tmp.c_str(), doc);
    ...
    for (size_t i = 0; i < parentQuery->terms->size(); i++)
    {
        ...
        docFreqs.push_back('=');
        docFreqs.append(std::to_wstring(searcher->docFreq(term)));

        query.append(term->text());
    }
    query.push_back('\"');

    _snwprintf(descbuf, LUCENE_SEARCH_EXPLANATION_DESC_LEN,
        L"idf(%s: %s)", parentQuery->field, docFreqs.c_str());
    ...
    tmp = getQuery()->toString();
    _snwprintf(descbuf, LUCENE_SEARCH_EXPLANATION_DESC_LEN,
        L"queryWeight(%s), product of:", tmp.c_str());
    ...
    _snwprintf(descbuf, LUCENE_SEARCH_EXPLANATION_DESC_LEN,
        L"fieldWeight(%s:%s in %d), product of:",
        parentQuery->field, query.c_str(), doc);
    ...
    fieldExpl->addDetail(_CLNEW Explanation(idfExpl->getValue(), idfExpl->getDescription().c_str()));
    ...
    _snwprintf(descbuf, LUCENE_SEARCH_EXPLANATION_DESC_LEN,
        L"fieldNorm(field=%s, doc=%d)", parentQuery->field, doc);
