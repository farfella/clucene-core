...
CL_NS_DEF(search)
  ...
  Explanation* TermScorer::explain(int32_t doc) {
	...
	wchar_t* termToString = query->getTerm(false)->toString();
	_snwprintf(buf,LUCENE_SEARCH_EXPLANATION_DESC_LEN, L"tf(termFreq(%s)=%d)", termToString, tf);
    _CLDELETE_LCARRAY(termToString);
    tfExplanation->setDescription(buf);
	return tfExplanation;
  }

  wchar_t* TermScorer::toString() { 
     wchar_t* wb = weight->toString();
     int32_t rl = wcslen(wb) + 9; //9=wcslen("scorer("  ")") + 1
     wchar_t* ret = _CL_NEWARRAY(wchar_t,rl);
	 _snwprintf(ret,rl,L"scorer(%s)", wb);
