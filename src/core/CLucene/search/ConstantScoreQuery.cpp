...
public:
    ...
    wchar_t* toString(){
        return _wcsdup(L"ConstantScorer");
    }
...
public:
    ...
    Explanation* explain(IndexReader* reader, int32_t doc) {
        ...
        if (exists) {
            ...
            buf.append(L"ConstantScoreQuery(");

            wchar_t* tmp = parentQuery->filter->toString();
            buf.append(tmp);
...
wchar_t* ConstantScoreQuery::toString(const wchar_t* /*field*/) const
{
    StringBuffer buf;
    buf.append(L"ConstantScore(");
    wchar_t* tmp = filter->toString();
    buf.append(tmp);
    _CLLDELETE(tmp);
...
wchar_t* ConstantScoreRangeQuery::toString(const wchar_t* field) const
{
