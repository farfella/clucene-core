...
void SpanNearQuery::setField( const wchar_t * field )
{
...
wchar_t* SpanNearQuery::toString( const wchar_t* field ) const
{
...
bool SpanNearQuery::equals( Query* other ) const
{
    ...
    if( inOrder != that->inOrder
        ...
        || 0 != wcscmp( field, that->field ) )     // CLucene: java version does not compare field names
    {
        return false;
    }
