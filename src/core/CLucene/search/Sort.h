...
public:
   ...
   */
   virtual ScoreDocComparator* newComparator (CL_NS(index)::IndexReader* reader, const wchar_t* fieldname) = 0;
};
...
public:
	virtual ScoreDocComparator* newComparator (CL_NS(index)::IndexReader* reader, const wchar_t* fieldname);
   ...
   */
   virtual CL_NS(util)::Comparable* getComparable (const wchar_t* termtext) = 0;
...
public:
  ...
  */
  SortField (const wchar_t* field);
  //SortField (const wchar_t* field, bool reverse);
   ...
   */
  SortField (const wchar_t* field, int32_t type, bool reverse); 

  /*
   SortField (wchar_t* field, Locale* locale) {
   SortField (wchar_t* field, Locale* locale, bool reverse);*/
   ...
   * @param reverse True if natural order should be reversed (default=false).
   */
  SortField (const wchar_t* field, SortComparatorSource* comparator, bool reverse=false);
...
public:
	...
	* slightly more overhead. */
	 ...
	 * @see SortField#AUTO
	 */
	Sort (const wchar_t* field, bool reverse=false);
	...
	void setSort (const wchar_t* field, bool reverse=false);
	
	/** Sets the sort to the terms in each field in succession. */
	void setSort (const wchar_t** fieldnames);
