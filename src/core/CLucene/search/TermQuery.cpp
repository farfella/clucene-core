...
const std::wstring TermQuery::getClassName()
{
    return L"TermQuery";
}
const std::wstring TermQuery::getObjectName() const
{
    return getClassName();
}
...
std::wstring TermQuery::toString(const wchar_t* field) const
{
    std::wstring buffer;
...
//
std::wstring TermWeight::toString()
{
    std::wstring tmp = L"weight(" + parentQuery->getObjectName() + L")";
    return tmp;
}
