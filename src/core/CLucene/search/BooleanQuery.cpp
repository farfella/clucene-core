...
const std::wstring BooleanQuery::getObjectName() const
{
    return getClassName();
}
const std::wstring BooleanQuery::getClassName()
{
    return L"BooleanQuery";
}
...
std::wstring BooleanQuery::toString(const wchar_t* field) const
{
    std::wstring buffer;
    ...
    for (uint32_t i = 0; i < clauses->size(); i++)
    {
        ...
        {	  // wrap sub-bools in parens
            ...
            std::wstring buf = c->getQuery()->toString(field);
            buffer.append(buf);
    ...
    {
        buffer.push_back(L'^');
        buffer.append(float_to_wstring(getBoost(), 1));
    }
    return buffer;
}
...
{
    CND_MESSAGE(false, L"Warning: BooleanQuery::getClauses() is deprecated")
...
Explanation* BooleanWeight::explain(IndexReader* reader, int32_t doc)
{
    ...
    for (size_t i = 0; i < weights.size(); i++)
    {
        ...
        if (e->isMatch())
        {
            ...
            {
                std::wstring buf = L"match on prohibited clause (";
                std::wstring tmp = c->getQuery()->toString();
                buf.append(tmp);
                buf.push_back(L')');

                Explanation* r = _CLNEW Explanation(0.0f, buf.c_str());
                r->addDetail(e);
                sumExpl->addDetail(r);
                fail = true;
            }
        ...
        {
            std::wstring buf = L"no match on required clause (";
            std::wstring tmp = c->getQuery()->toString();
            buf.append(tmp);
            buf.push_back(L')');
    ...
    else if (shouldMatchCount < minShouldMatch)
    {
        ...
        std::wstring buf = L"Failure to match minimum number of optional clauses: ";
        buf.append(std::to_wstring(minShouldMatch));
        sumExpl->setDescription(buf.c_str());
        return sumExpl;
    }
    ...
    else
    {
        ...
        std::wstring buf = L"coord(";
        buf.append(std::to_wstring(coord));
        buf.push_back(L'/');
        buf.append(std::to_wstring(maxCoord));
        buf.push_back(L')');
        result->addDetail(_CLNEW Explanation(coordFactor, buf.c_str()));
        return result;
    }
}
...
std::wstring BooleanClause::toString() const
{
    std::wstring buffer;
    if (occur == MUST)
        buffer.append(L"+");
...
void BooleanClause::setFields(Occur occur)
{
    ...
    {
        _CLTHROWT(CL_ERR_UnknownOperator, L"Unknown operator");
    }
}
