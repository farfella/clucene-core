...
private:
	...
	static ScoreDocComparator* lookup (CL_NS(index)::IndexReader* reader, const wchar_t* field, int32_t type, SortComparatorSource* factory);
	
	/** Stores a comparator into the cache. 
		returns the valid ScoreDocComparator.
	*/
	static void store (CL_NS(index)::IndexReader* reader, const wchar_t* field, int32_t type, SortComparatorSource* factory, ScoreDocComparator* value);

  
  //todo: Locale locale, not implemented yet
  static ScoreDocComparator* getCachedComparator (CL_NS(index)::IndexReader* reader, 
  	const wchar_t* fieldname, int32_t type, SortComparatorSource* factory);
   ...
   */
  static ScoreDocComparator* comparatorInt (CL_NS(index)::IndexReader* reader, const wchar_t* fieldname);
   ...
   * @throws IOException If an error occurs reading the index.
   */
  static ScoreDocComparator* comparatorFloat (CL_NS(index)::IndexReader* reader, const wchar_t* fieldname);
   ...
   * @throws IOException If an error occurs reading the index.
   */
  static ScoreDocComparator* comparatorString (CL_NS(index)::IndexReader* reader, const wchar_t* fieldname);
  ...
  static ScoreDocComparator* comparatorStringLocale (IndexReader* reader, wchar_t* fieldname, Locale locale){
    Collator collator = Collator.getInstance (locale);
    wchar_t* field = fieldname.intern();
    wchar_t** index = FieldCache.DEFAULT.getStrings (reader, field);
   ...
   * @throws IOException If an error occurs reading the index.
   */
  static ScoreDocComparator* comparatorAuto (CL_NS(index)::IndexReader* reader, const wchar_t* fieldname);
