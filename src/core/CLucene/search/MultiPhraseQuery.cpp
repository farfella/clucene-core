...
public:
    ...
    Explanation* explain(IndexReader* reader, int32_t doc)
    {
        ...
        std::wstring buf;
        buf.append(L"weight(");
        std::wstring queryString = getQuery()->toString();
        ...
        buf.append(queryString);
        buf.push_back(L')');
        Explanation* idfExpl = _CLNEW Explanation(idf, buf.c_str());
        buf.clear();

        // explain query weight
        Explanation* queryExpl = _CLNEW Explanation();
        buf.append(L"queryWeight(");
        buf.append(queryString);
        buf.append(L"), product of:");
        queryExpl->setDescription(buf.c_str());
        buf.clear();
        ...
        buf.append(L"fieldWeight(");
        buf.append(queryString);
        buf.append(L" in ");
        buf.append(std::to_wstring(doc));
        buf.append(L"), product of:");
        fieldExpl->setDescription(buf.c_str());
        buf.clear();
        ...
        buf.append(L"fieldNorm(field=");
        buf.append(parentQuery->field);
        buf.append(L", doc=");
        buf.append(std::to_wstring(doc));
        buf.push_back(L')');
        fieldNormExpl->setDescription(buf.c_str());
        buf.clear();
...
MultiPhraseQuery::MultiPhraseQuery(const MultiPhraseQuery& clone) :
    Query(clone)
{
    this->field = clone.field ? _wcsdup(clone.field) : NULL;
...
void MultiPhraseQuery::add(const CL_NS(util)::ArrayBase<CL_NS(index)::Term*>* _terms, const int32_t position)
{
    ...
    {
        if (wcscmp(_terms->values[i]->field(), field) != 0)
        {
            wchar_t buf[250];
            _snwprintf(buf, 250, L"All phrase terms must be in the same field (%s): %s", field, (*terms)[i]->field());
...
std::wstring MultiPhraseQuery::toString(const wchar_t* f) const
{
    std::wstring buffer;
    if (wcscmp(f, field) != 0)
    {
