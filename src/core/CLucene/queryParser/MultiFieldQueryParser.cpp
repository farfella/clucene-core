...
MultiFieldQueryParser::MultiFieldQueryParser(const wchar_t** _fields, CL_NS(analysis)::Analyzer* a, BoostMap* _boosts):
...
Query* MultiFieldQueryParser::getFieldQuery(const wchar_t* field, wchar_t* queryText, const int32_t slop){
	if (field == NULL) {
		...
		for (int i = 0; fields[i]!=NULL; ++i) {
            ...
            BoostMap::const_iterator itr = boosts->find((wchar_t*)fields[i]);
            if (itr != boosts->end()) {
...
Query* MultiFieldQueryParser::getFuzzyQuery(const wchar_t* field, wchar_t* termStr, const float_t minSimilarity){
	if (field == NULL) {
		vector<BooleanClause*> clauses;
		for (int i = 0; fields[i]!=NULL; ++i) {
...
Query* MultiFieldQueryParser::getPrefixQuery(const wchar_t* field, wchar_t* termStr){
	if (field == NULL) {
		vector<BooleanClause*> clauses;
		for (int i = 0; fields[i]!=NULL; ++i) {
...
Query* MultiFieldQueryParser::getWildcardQuery(const wchar_t* field, wchar_t* termStr){
	if (field == NULL) {
		vector<BooleanClause*> clauses;
		for (int i = 0; fields[i]!=NULL; ++i) {
...
Query* MultiFieldQueryParser::getRangeQuery(const wchar_t* field, wchar_t* part1, wchar_t* part2, const bool inclusive){
	if (field == NULL) {
		vector<BooleanClause*> clauses;
		for (int i = 0; fields[i]!=NULL; ++i) {
...
//static
Query* MultiFieldQueryParser::parse(const wchar_t** _queries, const wchar_t** _fields, Analyzer* analyzer)
{
...
// static
Query* MultiFieldQueryParser::parse(const wchar_t* query, const wchar_t** _fields, const uint8_t* flags, Analyzer* analyzer) {
	BooleanQuery* bQuery = _CLNEW BooleanQuery();
	for (size_t i = 0; _fields[i]!=NULL; i++) {
...
//static
Query* MultiFieldQueryParser::parse(const wchar_t** _queries, const wchar_t** _fields, const uint8_t* flags, Analyzer* analyzer){
	BooleanQuery* bQuery = _CLNEW BooleanQuery();
	for (size_t i = 0; _fields[i]!=NULL; i++)
	{
	  //TODO: this is really confusing... why not refactor _fields and flags to use a array object.
