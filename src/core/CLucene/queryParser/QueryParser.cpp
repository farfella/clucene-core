...
//static
std::wstring QueryParser::escape(const wchar_t* s)
{
    size_t len = wcslen(s);
...
void QueryParser::generateParseException()
{
    ...
    std::wstring err = getParseExceptionMessage(token, jj_expentries, tokenImage);
    _CLTHROWT(CL_ERR_Parse, err.c_str());
}
...
std::wstring QueryParserConstants::addEscapes(wchar_t* str)
{
...
std::wstring QueryParser::getParseExceptionMessage(QueryToken* currentToken,
    ...
    const wchar_t* tokenImage[])
{
    ...
    for (size_t i = 0; i < maxSize; i++)
    {
        ...
        if (tok->image)
        {
            std::wstring buf = addEscapes(tok->image);
            retval.append(buf);
        }
        tok = tok->next;
    }
    retval.append(L"\" at line ");
    retval.append(std::to_wstring(currentToken->next->beginLine));
    retval.append(L", column ");
    retval.append(std::to_wstring(currentToken->next->beginColumn));
    retval.push_back(L'.');
    retval.append(eol);
