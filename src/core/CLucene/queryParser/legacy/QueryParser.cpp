...
CL_NS_DEF2(queryParser,legacy)
    ...
    QueryParser::QueryParser(const wchar_t* _field, Analyzer* _analyzer) : QueryParserBase(_analyzer){
    //Func - Constructor.
    ...
    //static
    Query* QueryParser::parse(const wchar_t* query, const wchar_t* field, Analyzer* analyzer){
    ...
    Query* QueryParser::parse(const wchar_t* query){
	...
	Query* QueryParser::MatchQuery(const wchar_t* field){
	...
	Query* QueryParser::MatchClause(const wchar_t* field){
	...
	Query* QueryParser::MatchTerm(const wchar_t* field){
		...
		switch(term->Type){
			...
			{ //start case
				...
				if(wildcard){
					...
					term->Value[wcslen(term->Value)-1] = 0; //discard the *
					...
					if(term->Value[wcslen(term->Value)-1] == '~'){
						//remove the ~
						term->Value[wcslen(term->Value)-1] = '\0';
					}
			...
			case QueryToken::RANGEEX:{
				...
				wchar_t* noBrackets = term->Value + 1;
				noBrackets[wcslen(noBrackets)-1] = 0;
			...
			case QueryToken::QUOTED:{
				...
				//remove the quotes
				wchar_t* quotedValue = term->Value+1;
				quotedValue[wcslen(quotedValue)-1] = '\0';

				int32_t islop = phraseSlop;
				if(slop != NULL ){
				   try {
             islop = _wtoi(slop->Value+1);
				   }catch(...){
					   //ignored
				   }
				}
	...
	//Post -
	  ...
	  if (expectedType != t->Type){
		  wchar_t buf[200];
		  _snwprintf(buf,200, L"Error: Unexpected QueryToken: %d, expected: %d",t->Type,expectedType);
		  _CLDELETE(t);
