...
void QueryParserBase::discardEscapeChar(wchar_t* source) const{
  int len = wcslen(source);
  for (int i = 0; i < len; i++) {
    if (source[i] == '\\' && source[i+1] != '\0' ) {
      wcscpy(source+i,source+i+1);
      len--;
    }
  }
}
...
//Post -
  ...
  if ( required && prohibited )
    throwParserException( L"Clause cannot be both required and prohibited", ' ',0,0);
  clauses.push_back(_CLNEW BooleanClause(q,true, required, prohibited));
}

void QueryParserBase::throwParserException(const wchar_t* message, wchar_t ch, int32_t col, int32_t line )
{
  wchar_t msg[1024];
  _snwprintf(msg,1024,message,ch,col,line);
  _CLTHROWT (CL_ERR_Parse, msg );
}


Query* QueryParserBase::GetFieldQuery(const wchar_t* field, wchar_t* queryText, int32_t slop){
...
Query* QueryParserBase::GetFieldQuery(const wchar_t* field, wchar_t* queryText){
...
//Post - A query instance has been returned for the specified field
  ...
  try{
    while (source->next(&t)){
      v.push_back(_wcsdup(t.termBuffer()));
...
Query* QueryParserBase::ParseRangeQuery(const wchar_t* field, wchar_t* queryText, bool inclusive)
{
  ...
  do
  {
    ...
    if (tret)
    {
      if ( !from && wcscmp(t.termBuffer(),L"TO")==0 )
        continue;


      wchar_t* tmp = _wcsdup(t.termBuffer());
...
Query* QueryParserBase::GetPrefixQuery(const wchar_t* field, wchar_t* termStr){
...
Query* QueryParserBase::GetFuzzyQuery(const wchar_t* field, wchar_t* termStr){
...
CL_NS(search)::Query* QueryParserBase::GetRangeQuery(const wchar_t* field, wchar_t* part1, wchar_t* part2, bool inclusive){
