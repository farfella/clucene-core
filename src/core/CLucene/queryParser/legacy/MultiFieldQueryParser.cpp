...
MultiFieldQueryParser::MultiFieldQueryParser(const wchar_t** fields, CL_NS(analysis)::Analyzer* a, BoostMap* boosts):
	QueryParser(NULL,a)
{
	this->fields = fields;
    this->boosts = boosts;
...
//static
Query* MultiFieldQueryParser::parse(const wchar_t* query, const wchar_t** fields, Analyzer* analyzer)
{
...
//static
Query* MultiFieldQueryParser::parse(const wchar_t* query, const wchar_t** fields, const uint8_t* flags, Analyzer* analyzer)
{
    BooleanQuery* bQuery = _CLNEW BooleanQuery( true );
    int32_t i = 0;
    while ( fields[i] != NULL )
...
//not static
CL_NS(search)::Query* MultiFieldQueryParser::parse(const wchar_t* query) {
	return parse(query, this->fields, this->analyzer);
}

Query* MultiFieldQueryParser::GetFieldQuery(const wchar_t* field, wchar_t* queryText, int32_t slop){
	if (field == NULL) {
...
Query* MultiFieldQueryParser::GetFieldQuery(const wchar_t* field, wchar_t* queryText){
	return GetFieldQuery(field, queryText, 0);
}


CL_NS(search)::Query* MultiFieldQueryParser::GetFuzzyQuery(const wchar_t* field, wchar_t* termStr){
	if (field == NULL) {
		vector<BooleanClause*> clauses;
		for (int i = 0; fields[i]!=NULL; ++i) {
...
Query* MultiFieldQueryParser::GetRangeQuery(const wchar_t* field, wchar_t* part1, wchar_t* part2, bool inclusive){
	if (field == NULL) {
		vector<BooleanClause*> clauses;
		for (int i = 0; fields[i]!=NULL; ++i) {
