...
public:
	...
	*/
	...
	virtual void throwParserException(const wchar_t* message, wchar_t ch, int32_t col, int32_t line );
...
protected:
	...
	*/
	void discardEscapeChar(wchar_t* token) const;

    //Analyzes the expanded term termStr with the StandardFilter and the LowerCaseFilter.
    wchar_t* AnalyzeExpandedTerm(const wchar_t* field, wchar_t* termStr);
	...
	*/
	virtual CL_NS(search)::Query* GetFieldQuery(const wchar_t* field, wchar_t* queryText);

	/**
	* Delegates to GetFieldQuery(string, string), and adds slop onto phrasequery.
	* Can be used to remove slop functionality
	*/
	virtual CL_NS(search)::Query* GetFieldQuery(const wchar_t* field, wchar_t* queryText, int32_t slop);
	...
	* return NULL to disallow
	*/
    virtual CL_NS(search)::Query* GetPrefixQuery(const wchar_t* field, wchar_t* termStr);
	...
	* return NULL to disallow
	*/
    virtual CL_NS(search)::Query* GetWildcardQuery(const wchar_t* field, wchar_t* termStr);
	...
	* return NULL to disallow
	*/
    virtual CL_NS(search)::Query* GetFuzzyQuery(const wchar_t* field, wchar_t* termStr);
	...
	* return NULL to disallow
	*/
	virtual CL_NS(search)::Query* GetRangeQuery(const wchar_t* field, wchar_t* part1, wchar_t* part2, bool inclusive);
	virtual CL_NS(search)::Query* ParseRangeQuery(const wchar_t* field, wchar_t* str, bool inclusive);
};
...
* The syntax for query const wchar_t*s is as follows:
...
public:
	...
	* analyzer values.
	*/
	QueryParser(const wchar_t* field, CL_NS(analysis)::Analyzer* analyzer);
	~QueryParser();
	...
	*/
	virtual CL_NS(search)::Query* parse(const wchar_t* query);
	...
	* analyzer values.
	*/
	static CL_NS(search)::Query* parse(const wchar_t* query, const wchar_t* field, CL_NS(analysis)::Analyzer* analyzer);
...
private:
	...
	*/
	CL_NS(search)::Query* MatchQuery(const wchar_t* field);

	/**
	* matches for CLAUSE
	* CLAUSE ::= [TERM <COLON>] ( TERM | (<LPAREN> QUERY <RPAREN>))
	*/
	CL_NS(search)::Query* MatchClause(const wchar_t* field);
	...
	*/
	CL_NS(search)::Query* MatchTerm(const wchar_t* field);
