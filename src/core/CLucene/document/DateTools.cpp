...
wchar_t* DateTools::timeToString(const int64_t time, Resolution resolution /*= MILLISECOND_FORMAT*/) {
	wchar_t* buf = _CL_NEWARRAY(wchar_t, DATETOOLS_BUFFER_SIZE);
	timeToString(time, resolution, buf, DATETOOLS_BUFFER_SIZE);
	return buf;
}

void DateTools::timeToString(const int64_t time, Resolution resolution, wchar_t* buf, size_t bufLength)
{
    ...
    Misc::_cpycharToWide(abuf, buf, bufLength);
}

tm* DateTools::stringToDate(const wchar_t* dateString){
...
int64_t DateTools::stringToTime(const wchar_t* dateString) {
  tm s_time;
  memset(&s_time, 0, sizeof (s_time));
  s_time.tm_mday = 1;
  int32_t ms = 0;

  switch (wcslen(dateString)) {
  case 4: // YEAR_FORMAT
  {
    s_time.tm_year = _wtoi(dateString) - 1900;
  ...
  {
    wchar_t* tmpDate = _wcsdup(dateString);
    s_time.tm_mday = _wtoi(&tmpDate[6]);
    tmpDate[6] = 0;
    s_time.tm_mon = _wtoi(&tmpDate[4]) - 1;
    tmpDate[4] = 0;
    s_time.tm_year = _wtoi(tmpDate) - 1900;
  ...
  {
    wchar_t* tmpDate = _wcsdup(dateString);
    s_time.tm_hour = _wtoi(&tmpDate[8]);
    tmpDate[8] = 0;
    s_time.tm_mday = _wtoi(&tmpDate[6]);
    tmpDate[6] = 0;
    s_time.tm_mon = _wtoi(&tmpDate[4]) - 1;
  ...
  {
    wchar_t* tmpDate = _wcsdup(dateString);
    s_time.tm_min = _wtoi(&tmpDate[10]);
    tmpDate[10] = 0;
    s_time.tm_hour = _wtoi(&tmpDate[8]);
    tmpDate[8] = 0;
    s_time.tm_mday = _wtoi(&tmpDate[6]);
  ...
  {
    wchar_t* tmpDate = _wcsdup(dateString);
    s_time.tm_sec = _wtoi(&tmpDate[12]);
    tmpDate[12] = 0;
    s_time.tm_min = _wtoi(&tmpDate[10]);
    tmpDate[10] = 0;
    s_time.tm_hour = _wtoi(&tmpDate[8]);
...
wchar_t* DateTools::getISOFormat(const int64_t time){
...
void DateTools::strCatDate(wchar_t* buf, int zeroes, int value){
  wchar_t str[10];
...
wchar_t* DateTools::getISOFormat(unsigned short year, uint8_t month, uint8_t mday, uint8_t hours,
        uint8_t minutes, uint8_t seconds, unsigned short ms)
{
    wchar_t* ISOString = _CL_NEWARRAY(wchar_t, 24);
    wchar_t* p = ISOString;
