...
private:
  static void strCatDate(wchar_t* buf, int zeroes, int value);
public:
  ...
  */
  static wchar_t* timeToString(const int64_t time, Resolution resolution = MILLISECOND_FORMAT);

  static void timeToString(const int64_t time, Resolution resolution, wchar_t* buf, size_t bufLength);
  ...
  *  expected format
  */
  static int64_t stringToTime(const wchar_t* dateString);

  static tm* stringToDate(const wchar_t* dateString);
  ...
  */
  ...
  static wchar_t* getISOFormat(const int64_t time);
  static wchar_t* getISOFormat(unsigned short year, uint8_t month, uint8_t mday, uint8_t hours = 0,
      uint8_t minutes = 0, uint8_t seconds = 0, unsigned short ms = 0);
