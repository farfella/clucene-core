...
public:
	...
	*/
	_CL_DEPRECATED( DateTools ) static wchar_t* timeToString(const int64_t time);
	...
	*/
	_CL_DEPRECATED( DateTools ) static void timeToString(const int64_t time, wchar_t* str);
	
	/** Converts a string-encoded date into a millisecond time. */
	_CL_DEPRECATED( DateTools ) static int64_t stringToTime(const wchar_t* s);
};
CL_NS_END
#endif
