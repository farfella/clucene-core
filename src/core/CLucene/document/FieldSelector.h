...
public:
  ...
  **/
  enum FieldSelectorResult {
	...
	*/
	virtual FieldSelectorResult accept(const wchar_t* fieldName) const = 0;
...
public:
	...
	FieldSelectorResult accept(const wchar_t* fieldName) const;
};
...
public:
  typedef CL_NS(util)::CLHashMap<wchar_t*, FieldSelectorResult,
  ...
  MapFieldSelector(std::vector<const wchar_t*>& fieldSelections);
   ...
   */
  MapFieldSelector(CL_NS(util)::ArrayBase<const wchar_t*>& fields);

  /** Load field according to its associated value in fieldSelections
   * @param field a field name
   * @return the fieldSelections value that field maps to or NO_LOAD if none.
   */
  FieldSelectorResult accept(const wchar_t* field) const;

  void add(const wchar_t*, FieldSelector::FieldSelectorResult action=FieldSelector::LOAD);
};
