...
public:
    ...
    */
	...
	/**
	* wchar_t value constructor of Field.
	* @memory Set duplicateValue to false to save on memory allocations when possible
	*/
	Field(const wchar_t* name, const wchar_t* value, int _config, const bool duplicateValue = true);

	/**
	* Reader* constructor of Field.
	* @memory consumes reader
	*/
	Field(const wchar_t* name, CL_NS(util)::Reader* reader, int _config);

	/**
	* Binary constructor of Field.
	* @memory Set duplicateValue to false to save on memory allocations when possible
	*/
  Field(const wchar_t* name, CL_NS(util)::ValueArray<uint8_t>* data, int _config, const bool duplicateValue = true);

	Field(const wchar_t* name, int _config); ///<No value, for lazy loading support
  virtual ~Field();

	/**  The name of the field (e.g., "date", "subject", "title", "body", etc.)
	*	as an interned string. */
	const wchar_t* name() const; ///<returns reference

	/** The value of the field as a String, or null.  If null, the Reader value
	* or binary value is used.  Exactly one of stringValue(), readerValue() and
	* binaryValue() must be set. */
	virtual const wchar_t* stringValue(); ///<returns reference
	...
	*  vector, available from {@link IndexReader#getTermFreqVector(int32_t,wchar_t*)}.
	...
	* @memory Caller is responsible for releasing value if duplicateValue == false */
	void setValue(wchar_t* value, const bool duplicateValue = true);

	/** Expert: change the value of this field.  See <a href="#setValue(wchar_t*)">setValue(wchar_t*)</a>. */
