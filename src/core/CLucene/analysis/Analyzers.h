...
private:
	...
	wchar_t buffer[LUCENE_MAX_WORD_LEN+1];
	const wchar_t* ioBuffer;
protected:
    ...
    * define token boundaries and are not included in tokens. */
	virtual bool isTokenChar(const wchar_t c) const = 0;

    /** Called on each token character to normalize it before it is added to the
    * token.  The default implementation does nothing.  Subclasses may use this
    * to, e.g., lowercase tokens. */
   	virtual wchar_t normalize(const wchar_t c) const;
...
protected:
	...
	wchar_t normalize(const wchar_t chr) const;
};
...
public:
	...
	StopFilter(TokenStream* in, bool deleteTokenStream, const wchar_t** _stopWords, const bool _ignoreCase = false);
	...
	*/
	static void fillStopTable(CLTCSetList* stopTable,
                                      const wchar_t** stopWords, const bool _ignoreCase = false);
...
public:
    ...
    StopAnalyzer( const wchar_t** stopWords );
	...
	*/
    ...
    for searching. */
    static const wchar_t* ENGLISH_STOP_WORDS[];
};
...
private:
    ...
    typedef CL_NS(util)::CLHashMap<wchar_t*, Analyzer*, CL_NS(util)::Compare::WChar,
    	CL_NS(util)::Equals::WChar, CL_NS(util)::Deletor::tcArray,CL_NS(util)::Deletor::Void<Analyzer> > AnalyzerMapType;
...
public:
    ...
    */
    void addAnalyzer(const wchar_t* fieldName, Analyzer* analyzer);
    TokenStream* tokenStream(const wchar_t* fieldName, CL_NS(util)::Reader* reader);

	TokenStream* reusableTokenStream(const wchar_t* fieldName, CL_NS(util)::Reader* reader);

	/** Return the positionIncrementGap from the analyzer assigned to fieldName */
	int32_t getPositionIncrementGap(const wchar_t* fieldName);
};
