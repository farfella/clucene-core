...
typedef CL_NS(util)::CLSetList<wchar_t*, CL_NS(util)::Compare::WChar, CL_NS(util)::Deletor::tcArray> CLTCSetList;
...
private:
	...
	const wchar_t* _type;				  ///< lexical type
	int32_t positionIncrement;
	size_t bufferTextLen;

	#ifndef LUCENE_TOKEN_WORD_LENGTH
	wchar_t* _buffer;				  ///< the text of the term
	#else
	wchar_t _buffer[LUCENE_TOKEN_WORD_LENGTH+1];				  ///< the text of the term
	#endif
...
public:
	static const wchar_t* getDefaultType();

	Token();
	virtual ~Token();

	/// Constructs a Token with the given text, start and end offsets, & type. 
	Token(const wchar_t* text, const int32_t start, const int32_t end, const wchar_t* typ=NULL);
	void set(const wchar_t* text, const int32_t start, const int32_t end, const wchar_t* typ=NULL);

	size_t bufferLength();
	void growBuffer(size_t size);
  wchar_t* resizeTermBuffer(size_t size);
	...
	*  characters that were placed into the termBuffer. */
	wchar_t* termBuffer() const;
	size_t termLength(); //< Length of the the termBuffer. See #termBuffer

	_CL_DEPRECATED( termBuffer ) const wchar_t* termText() const; //< See #termBuffer()
	...
	void setText(const wchar_t* txt, int32_t len=-1); //< Sets the termBuffer. See #termBuffer
  ...
  void setTermLength(int32_t);
	...
	*/
	...
	const wchar_t* type() const; ///<returns reference
	void setType(const wchar_t* val); ///<returns reference
...
public:
	...
	virtual TokenStream* tokenStream(const wchar_t* fieldName, CL_NS(util)::Reader* reader)=0;
	...
	*  performance.
	*/
	virtual TokenStream* reusableTokenStream(const wchar_t* fieldName, CL_NS(util)::Reader* reader);
private:
...
public:
	...
	* @return position increment gap, added to the next token emitted from {@link #tokenStream(wchar_t*, Reader*)}
	*/
	virtual int32_t getPositionIncrementGap(const wchar_t* fieldName);
