...
StringReader::StringReader(const wchar_t* _value, const int32_t _length, bool copyData)
{
    this->m_size = 0;
    this->value = NULL;
    this->ownValue = true;
...
void StringReader::init(const wchar_t* _value, const int32_t _length, bool copyData)
{
    const size_t length = (_length < 0 ? wcslen(_value) : _length);
    this->pos = 0;
    if (copyData)
    {
        wchar_t* tmp = (wchar_t*) this->value;
        if (tmp == NULL || !this->ownValue)
        {
            tmp = _CL_NEWARRAY(wchar_t, length + 1);
            this->buffer_size = length;
        }
        else if (length > this->buffer_size || length < (this->buffer_size / 2))
        { //expand, or shrink
            tmp = (wchar_t*) realloc(tmp, sizeof(wchar_t) * (length + 1));
            this->buffer_size = length;
        }
        wcsncpy(tmp, _value, length + 1);
        this->value = tmp;
    }
    else
    {
        if (ownValue && this->value != NULL)
            _CLDELETE_LARRAY((wchar_t*) this->value);
        this->value = _value;
        this->buffer_size = 0;
    }
    this->m_size = length;
...
StringReader::~StringReader()
{
    if (ownValue && this->value != NULL)
    {
        wchar_t* value = (wchar_t*) this->value;
...
int32_t StringReader::read(const wchar_t*& start, int32_t min, int32_t max)
{
    if (m_size == pos)
        return -1;
...
public:
    ...
    Internal(const wchar_t * path, int32_t buffersize)
    {
        int32_t fhandle = _wopen(path, _O_BINARY | O_RDONLY | _O_RANDOM, _S_IREAD);
...
FileInputStream::FileInputStream(const wchar_t * path, int32_t buflen)
{
    if (buflen == -1)
...
FileReader::FileReader(const wchar_t *path, const wchar_t *enc, int32_t buflen)
{
    int encoding;
    if (wcscmp(enc, L"ASCII") == 0)
        encoding = ASCII;
#ifdef _UCS2
    else if (wcscmp(enc, L"UTF-8") == 0)
        encoding = UTF8;
    else if (wcscmp(enc, L"UCS-2LE") == 0)
        encoding = UCS2_LE;
#endif
    else
...
FileReader::FileReader(const wchar_t *path, int encoding, int32_t buflen)
{
    init(_CLNEW FileInputStream(path, buflen), encoding);
}
...
int32_t SimpleInputStreamReader::read(const wchar_t*& start, int32_t min, int32_t max)
{
    return _internal->jsbuffer->read(start, min, max);
}
...
int32_t FilteredBufferedReader::read(const wchar_t*& start, int32_t min, int32_t max)
{
    return _internal->jsbuffer->read(start, min, max);
}
