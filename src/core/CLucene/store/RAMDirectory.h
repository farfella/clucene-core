...
{
protected:
    typedef CL_NS(util)::CLHashMap<wchar_t *, RAMFile*,
        CL_NS(util)::Compare::WChar, CL_NS(util)::Equals::WChar,
        CL_NS(util)::Deletor::tcArray, CL_NS(util)::Deletor::Object<RAMFile> > FileMap;
    /// Removes an existing file in the directory.
    virtual bool doDeleteFile(const wchar_t * name);
...
public:
     ...
     */
    RAMDirectory(const wchar_t * dir);
    ...
    /// Removes an existing file in the directory.
    virtual void renameFile(const wchar_t* from, const wchar_t * to);
    ...
    bool openInput(const wchar_t * name, IndexInput*& ret, CLuceneError& error, int32_t bufferSize = -1);
