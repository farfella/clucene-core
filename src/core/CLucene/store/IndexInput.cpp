...
CL_NS_USE(util)
  ...
  int32_t IndexInput::readString(char* buffer, const int32_t maxLength){
  	wchar_t* buf = _CL_NEWARRAY(wchar_t,maxLength);
    int32_t ret = -1;
  	try{
	  	ret = readString(buf,maxLength);
        CL_NS(util)::Misc::_cpywideToChar(buf, buffer, ret + 1);
        // STRCPY_WtoA(target,src,len)
  	}_CLFINALLY ( _CLDELETE_CARRAY(buf); )
  	return ret;
  }
  #endif

  int32_t IndexInput::readString(wchar_t* buffer, const int32_t maxLength){
   ...
   wchar_t* IndexInput::readString(){
    int32_t len = readVInt();

    if ( len == 0){
      return _wcsdup(LUCENE_BLANK_STRING);
    }
  ...
  void IndexInput::readChars( wchar_t* buffer, const int32_t start, const int32_t len) {
...
BufferedIndexInput::BufferedIndexInput(int32_t _bufferSize):
    ...
    bufferPosition(other.bufferPosition)
  {
    ...
    **   if ( clone.buffer != NULL) { */
	...
	const std::wstring BufferedIndexInput::getObjectName(){ return getClassName(); }
	const std::wstring BufferedIndexInput::getClassName(){ return L"BufferedIndexInput"; }
