...
*/
static CL_NS(util)::CLHashMap<const wchar_t *, FSDirectory*, CL_NS(util)::Compare::WChar, CL_NS(util)::Equals::WChar> DIRECTORIES(false, false);
...
* TODO: get rid of this and dup/fctnl or something like that...
    */
    ...
    public:
        ...
        wchar_t  path[CL_MAX_DIR]; //todo: this is only used for cloning, better to get information from the fhandle
        SharedHandle(const wchar_t * path);
        ~SharedHandle();
    };
    SharedHandle* handle;
    int64_t _pos;
...
public:
    static bool open(const wchar_t * path, IndexInput*& ret, CLuceneError& error, int32_t bufferSize = -1);
    ~FSIndexInput();
    ...
    const std::wstring getDirectoryType() const { return FSDirectory::getClassName(); }
    const std::wstring getObjectName() const { return getClassName(); }
    static const std::wstring getClassName() { return L"FSIndexInput"; }
protected:
...
public:
    FSIndexOutput(const wchar_t* path, int filemode);
    ~FSIndexOutput();
...
bool FSDirectory::FSIndexInput::open(const wchar_t * path, IndexInput*& ret, CLuceneError& error, int32_t __bufferSize)
{
    //Func - Constructor.
    ...
    //Open the file
    handle->fhandle = ::_wopen(path, _O_BINARY | O_RDONLY | _O_RANDOM, _S_IREAD);
...
FSDirectory::FSIndexInput::SharedHandle::SharedHandle(const wchar_t * path)
{
    fhandle = 0;
    _length = 0;
    _fpos = 0;
...
{
    CND_PRECONDITION(position >= 0 && position < handle->_length, L"Seeking out of range")
        _pos = position;
}
...
{
    CND_PRECONDITION(handle != NULL, L"shared file handle has closed");
    CND_PRECONDITION(handle->fhandle >= 0, L"file is not open");
...
FSDirectory::FSIndexOutput::FSIndexOutput(const wchar_t* path, int filemode)
{
    ...
    if (Misc::dir_Exists(path))
        fhandle = _wopen(path, _O_BINARY | O_RDWR | _O_RANDOM | O_TRUNC, filemode);
    else // added by JBP
        fhandle = _wopen(path, _O_BINARY | O_RDWR | _O_RANDOM | O_CREAT, filemode);
...
{
    CND_PRECONDITION(fhandle >= 0, L"file is not open");
...
const wchar_t* FSDirectory::LOCK_DIR = NULL;
const wchar_t* FSDirectory::getLockDir()
{
    ...
    if (LOCK_DIR == NULL)
        LOCK_DIR = _wgetenv(LUCENE_LOCK_DIR_ENV_1);
#endif
#ifdef LUCENE_LOCK_DIR_ENV_2
    if (LOCK_DIR == NULL)
        LOCK_DIR = _wgetenv(LUCENE_LOCK_DIR_ENV_2);
#endif
...
void FSDirectory::init(const wchar_t* _path, LockFactory* lockFactory)
{
    directory = _path;
    ...
    {
        wchar_t * err = _CL_NEWARRAY(wchar_t, 19 + directory.length() + 1); //19: len of " is not a directory"
        wcscpy(err, directory.c_str());
        wcscat(err, L" is not a directory");
        _CLTHROWT_DEL(CL_ERR_IO, err);
    }
}
...
void FSDirectory::create()
{
    ...
    while (itr != files.end())
    {
        ...
        {
            if (_wunlink((directory + PATH_DELIMITERW + *itr).c_str()) == -1)
            {
...
void FSDirectory::priv_getFN(wchar_t * buffer, const wchar_t* name) const
{
    buffer[0] = 0;
    wcscpy(buffer, directory.c_str());
    wcscat(buffer, PATH_DELIMITERW);
    wcscat(buffer, name);
}
...
const std::wstring FSDirectory::getClassName()
{
    return L"FSDirectory";
}
const std::wstring FSDirectory::getObjectName() const
{
    return getClassName();
}
...
bool FSDirectory::list(std::vector<std::wstring>* names) const
{ //todo: fix this, ugly!!!
    CND_PRECONDITION(!directory.empty(), L"directory is not open");
    return Misc::listFiles(directory.c_str(), *names, false);
}

bool FSDirectory::fileExists(const wchar_t * name) const
{
    CND_PRECONDITION(directory[0] != 0, L"directory is not open");
    wchar_t fl[CL_MAX_DIR];
    priv_getFN(fl, name);
    return Misc::dir_Exists(fl);
}

const wchar_t * FSDirectory::getDirName() const
{
    return directory.c_str();
}

FSDirectory* FSDirectory::getDirectory(const wchar_t * file, bool create, LockFactory* lockFactory)
{
...
//static
FSDirectory* FSDirectory::getDirectory(const wchar_t* _file, LockFactory* lockFactory)
{
    FSDirectory* dir = NULL;
    {
        ...
        wchar_t buf[CL_MAX_PATH];
        wchar_t * file = _wfullpath(buf, _file, CL_MAX_PATH);//set a realpath so that if we change directory, we can still function
        if (!file || !*file)
        {
            wcsncpy(buf, _file, CL_MAX_PATH);
            file = buf;
        }

        struct cl_stat_t fstat;
        if (_wstati64(file, &fstat) == 0 && !(fstat.st_mode & S_IFDIR))
        {
            wchar_t tmp[1024];
            _snwprintf(tmp, 1024, L"%s not a directory", file);
            _CLTHROWA(CL_ERR_IO, tmp);
        }

        if (_wstati64(file, &fstat) != 0)
        {
            //todo: should construct directory using _mkdirs... have to write replacement
            if (_wmkdir(file) == -1)
            {
                std::wstring err = L"Couldn't create directory: ";
                err += std::wstring(file);
...
int64_t FSDirectory::fileModified(const wchar_t * name) const
{
    CND_PRECONDITION(directory[0] != 0, L"directory is not open");
    struct cl_stat_t buf;
    wchar_t buffer[CL_MAX_DIR];
    priv_getFN(buffer, name);
    if (_wstati64(buffer, &buf) == -1)
        return 0;
    else
        return buf.st_mtime;
}
...
//static
int64_t FSDirectory::fileModified(const wchar_t * dir, const wchar_t * name)
{
    struct cl_stat_t buf;
    wchar_t buffer[CL_MAX_DIR];
    _snwprintf(buffer, CL_MAX_DIR, L"%s%s%s", dir, PATH_DELIMITERW, name);
    _wstati64(buffer, &buf);
    return buf.st_mtime;
}

void FSDirectory::touchFile(const wchar_t * name)
{
    CND_PRECONDITION(directory[0] != 0, L"directory is not open");
    wchar_t buffer[CL_MAX_DIR];
    _snwprintf(buffer, CL_MAX_DIR, L"%s%s%s", directory.c_str(), PATH_DELIMITERW, name);

    int32_t r = _wopen(buffer, O_RDWR, this->filemode);
    if (r < 0)
        _CLTHROWA(CL_ERR_IO, "IO Error while touching file");
    ::_close(r);
}

int64_t FSDirectory::fileLength(const wchar_t * name) const
{
    CND_PRECONDITION(directory[0] != 0, L"directory is not open");
    struct cl_stat_t buf;
    wchar_t buffer[CL_MAX_DIR];
...
bool FSDirectory::openInput(const wchar_t * name, IndexInput *& ret, CLuceneError& error, int32_t bufferSize)
{
    CND_PRECONDITION(directory[0] != 0, L"directory is not open")
        wchar_t fl[CL_MAX_DIR];
    priv_getFN(fl, name);
...
wchar_t * FSDirectory::getLockPrefix() const
{
    wchar_t dirName[CL_MAX_PATH]; // name to be hashed
    if (_wfullpath(dirName, directory.c_str(), CL_MAX_PATH) == NULL)
    {
    ...
    if (dirName[1] == ':')
        dirName[0] = (WCHAR) _totupper((WCHAR) dirName[0]);

    wchar_t * smd5 = MD5String(dirName);

    wchar_t * ret = _CL_NEWARRAY(wchar_t, 32 + 7 + 1); //32=2*16, 7=strlen("lucene-")
    wcscpy(ret, L"lucene-");
    wcscat(ret, smd5);
...
bool FSDirectory::doDeleteFile(const wchar_t * name)
{
    CND_PRECONDITION(directory[0] != 0, L"directory is not open");
    wchar_t fl[CL_MAX_DIR];
    priv_getFN(fl, name);
    return _wunlink(fl) != -1;
}

void FSDirectory::renameFile(const wchar_t * from, const wchar_t * to)
{
    CND_PRECONDITION(directory[0] != 0, L"directory is not open");
    SCOPED_LOCK_MUTEX(THIS_LOCK)
    wchar_t old[CL_MAX_DIR];
    priv_getFN(old, from);

    wchar_t nu[CL_MAX_DIR];
    priv_getFN(nu, to);
    ...
    if (Misc::dir_Exists(nu))
    {
        ...
        {
            wchar_t* err = _CL_NEWARRAY(wchar_t, 16 + wcslen(to) + 1); //16: len of "couldn't delete "
            wcscpy(err, L"couldn't delete ");
            wcscat(err, to);
            _CLTHROWT_DEL(CL_ERR_IO, err);
        }
    }
    if (_wrename(old, nu) != 0)
    {
        ...
        wchar_t buffer[20 + CL_MAX_PATH + CL_MAX_PATH];
        wcscpy_s(buffer, L"couldn't rename ");
        wcscat_s(buffer, from);
        wcscat_s(buffer, L" to ");
        wcscat_s(buffer, nu);
        _CLTHROWT(CL_ERR_IO, buffer);
    }
}

IndexOutput* FSDirectory::createOutput(const wchar_t * name)
{
    CND_PRECONDITION(directory[0] != 0, L"directory is not open");
    wchar_t fl[CL_MAX_DIR];
    priv_getFN(fl, name);
    if (Misc::dir_Exists(fl))
    {
        ...
        {
            wchar_t tmp[1024];
            wcscpy_s(tmp, L"Cannot overwrite: ");
            wcscat_s(tmp, name);
            _CLTHROWA(CL_ERR_IO, tmp);
        }
...
std::wstring FSDirectory::toString() const
{
    return std::wstring(L"FSDirectory@") + this->directory;
}
