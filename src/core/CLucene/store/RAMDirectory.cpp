...
const std::wstring RAMInputStream::getDirectoryType() const
{
    return RAMDirectory::getClassName();
}

const std::wstring RAMIndexInput::getObjectName() const { return getClassName(); }

const std::wstring RAMIndexInput::getClassName() { return L"RAMIndexInput"; }
...
bool RAMDirectory::list(std::vector<std::wstring>* names) const
{
    ...
    while (itr != files->end())
    {
        names->push_back(std::wstring(itr->first));
        ++itr;
    }
    return true;
}
...
RAMDirectory::RAMDirectory(const wchar_t * dir) :
    Directory(), files(_CLNEW FileMap(true, true))
{
    this->sizeInBytes = 0;
    setLockFactory(_CLNEW SingleInstanceLockFactory());
...
bool RAMDirectory::fileExists(const wchar_t * name) const
{
    SCOPED_LOCK_MUTEX(files_mutex);
    return files->exists((wchar_t *) name);
}

int64_t RAMDirectory::fileModified(const wchar_t * name) const
{
    SCOPED_LOCK_MUTEX(files_mutex);
    RAMFile* f = files->get((wchar_t *) name);
    return f->getLastModified();
}

int64_t RAMDirectory::fileLength(const wchar_t * name) const
{
    SCOPED_LOCK_MUTEX(files_mutex);
    RAMFile* f = files->get((wchar_t *) name);
    return f->getLength();
...
bool RAMDirectory::openInput(const wchar_t * name, IndexInput*& ret, CLuceneError& error, int32_t /*bufferSize*/)
{
    SCOPED_LOCK_MUTEX(files_mutex);
    RAMFile* file = files->get((wchar_t *) name);
    if (file == NULL)
    {
...
bool RAMDirectory::doDeleteFile(const wchar_t * name)
{
    SCOPED_LOCK_MUTEX(files_mutex);
    FileMap::iterator itr = files->find((wchar_t *) name);
    if (itr != files->end())
    {
...
void RAMDirectory::renameFile(const wchar_t * from, const wchar_t * to)
{
    SCOPED_LOCK_MUTEX(files_mutex);
    FileMap::iterator itr = files->find((wchar_t *) from);
    ...
    ** IndexWriter.addIndexes with the file named 'segments'). */
    if (files->exists((wchar_t *) to))
    {
        FileMap::iterator itr1 = files->find((wchar_t *) to);
        SCOPED_LOCK_MUTEX(this->THIS_LOCK);
    ...
    {
        wchar_t tmp[1024];
        _snwprintf(tmp, 1024, L"cannot rename %s, file does not exist", from);
        _CLTHROWT(CL_ERR_IO, tmp);
    }
    CND_PRECONDITION(itr != files->end(), L"itr==files->end()")
        RAMFile* file = itr->second;
    files->removeitr(itr, false, true);
    files->put(_wcsdup(to), file);
}


void RAMDirectory::touchFile(const wchar_t * name)
{
    RAMFile* file = NULL;
    {
        SCOPED_LOCK_MUTEX(files_mutex);
        file = files->get((wchar_t *) name);
    }
...
IndexOutput* RAMDirectory::createOutput(const wchar_t * name)
{
    ...
    ** to $files. */
    ...
    wchar_t * n = NULL;
    FileMap::const_iterator itr = files->find(const_cast<wchar_t *>(name));
    if (itr != files->end())
    {
        n = itr->first;
...
std::wstring RAMDirectory::toString() const
{
    return L"RAMDirectory";
}

const std::wstring RAMDirectory::getClassName()
{
    return L"RAMDirectory";
}
const std::wstring RAMDirectory::getObjectName() const
{
    return getClassName();
}
