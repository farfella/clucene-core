...
CL_NS_DEF(store)
  ...
  void IndexOutput::writeString(const char* s, const int32_t length ) {
  	wchar_t* buf = _CL_NEWARRAY(wchar_t,length+1);
    Misc::_cpycharToWide(s, buf, length);
  	try{
  ...
  void IndexOutput::writeString(const wchar_t* s, const int32_t length ) {
    writeVInt(length);
    writeChars(s, length);
  }

  void IndexOutput::writeChars(const wchar_t* s, const int32_t length){
    if ( length < 0 )
