...
CL_NS_DEF(store)
  ...
  const char* FSLock::getObjectName() const{
   ...
   bool FSLock::obtain()
   {
	      ...
	      if ( _mkdir(lockDir) == -1 ){
              const size_t sz = 34 + strlen(lockDir) + 1;
	   		  char* err = _CL_NEWARRAY(char, sz); //34: len of "Couldn't create lock directory: "
	   		  strcpy_s(err, sz, "Couldn't create lock directory: ");
	   		  strcat_s(err, sz, lockDir);
