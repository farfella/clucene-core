...
//this is global...
void Misc::Sleep(DWORD dwMilliseconds)
{
...
std::string Misc::toString(const wchar_t* s, int32_t len) {
    if (s == NULL || len == 0) return "";
    if (len < 0) len = wcslen(s);
    char* buf = _CL_NEWARRAY(char, len + 1);

    _cpywideToChar(s, buf, len + 1);
...
//static
bool Misc::priv_isDotDir(const wchar_t * name)
{
    if (name[0] == '\0') {
        return (false);
    }
...
void Misc::segmentname(char* buffer, int32_t bufferLen, const char* Segment, const char* ext, const int32_t x) {
    ...
    if (x == -1)
        _snprintf_s(buffer, bufferLen, bufferLen, "%s%s", Segment, ext);
    else
        _snprintf_s(buffer, bufferLen, bufferLen, "%s%s%d", Segment, ext, x);
}
...
{
    char buf[20];
    _snprintf_s(buf, 20, "%0.2f", (double)value);
    return buf;
}
