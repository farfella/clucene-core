    ...
    if (t == G_UNICODE_UPPERCASE_LETTER)
    {
        ...
        for (i = 0; i < G_N_ELEMENTS(title_table); ++i)
        {
            if (title_table[i][0] == c)
                return (wchar_t)title_table[i][2];
        }
    }
    return (wchar_t)c;
}
...
wchar_t cl_toupper(wchar_t ch)
{
    ...
    else if (t == G_UNICODE_TITLECASE_LETTER)
    {
        ...
        {
            if (title_table[i][0] == c)
                return (wchar_t)title_table[i][1];
        }
    }
 ...
 **/
wchar_t cl_tcasefold(const wchar_t ch) {
    int start = 0;
...
wchar_t* cl_tcscasefold(wchar_t * str, int len) //len default is -1
{
    wchar_t *p = str;
...
//this function was not taken from gnome
int cl_tcscasefoldcmp(const wchar_t * dst, const wchar_t * src) {
    wchar_t f, l;
