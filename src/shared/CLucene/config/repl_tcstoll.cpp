...
int64_t lucene_tcstoi64(const wchar_t* str, wchar_t**end, int radix){
	#define LUCENE_TCSTOI64_RADIX(x,r) ((x>=L'0' && x<=L'9')?x-L'0':((x>=L'a' && x<=L'z')?x-L'a'+10:((x>=L'A' && x<=L'Z')?x-L'A'+10:1000)))
