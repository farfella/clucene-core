    ...
    */
    ...
    CL_NS(analysis)::Token* next(CL_NS(analysis)::Token* token)
    {
        ...
        else
        {
            ...
            {
                if (wcscmp(token->termBuffer(), _T("phrase")) == 0)
                {
                    inPhrase = true;
                ...
                else if (wcscmp(token->termBuffer(), _T("stop")) != 0)
                {
                    return token;
                }
            }
...
public:
    ...
    TokenStream* tokenStream(const wchar_t* /*fieldName*/, Reader* reader)
    {
...
{
public:
    QPTestParser(wchar_t* f, Analyzer* a) : QueryParser(f, a)
    {
    }
...
protected:
    Query * getFuzzyQuery(wchar_t* /*field*/, wchar_t* /*termStr*/, float_t /*minSimilarity*/)
    {
        _CLTHROWA(CL_ERR_Parse, "Fuzzy queries not allowed");
    }

    Query* getWildcardQuery(wchar_t* /*field*/, wchar_t* /*termStr*/)
    {
...
Query* getQuery(CuTest *tc, const wchar_t* query, Analyzer* a, int ignoreCLError = 0)
{
    bool del = (a == NULL);
    QueryParser* qp = NULL;
    try
...
void assertQueryEquals(CuTest* tc, QueryParser* qp, const wchar_t* field, const wchar_t* query, const wchar_t* result)
{
    Query* q = qp->parse(query);
    const wchar_t* s = q->toString(field).c_str();
    _CLLDELETE(q);
    if (wcscmp(s, result) != 0)
    {
        wchar_t str[CL_MAX_PATH];
        wcscpy(str, s);
        _CLLDELETE(qp);
...
void assertQueryEquals(CuTest *tc, const wchar_t* query, Analyzer* a, const wchar_t* result)
{
    ...
    const wchar_t* s = q->toString(_T("field")).c_str();
    int ret = wcscmp(s, result);
    _CLDELETE(q);
    if (ret != 0)
    {
        wchar_t str[CL_MAX_PATH];
...
void assertWildcardQueryEquals(CuTest *tc, const wchar_t* query, bool lowercase, const wchar_t* result, bool allowLeadingWildcard = false)
{
    SimpleAnalyzer a;
    ...
    std::wstring s = q->toString(_T("field"));
    _CLLDELETE(q);
    if (wcscmp(s.c_str(), result) != 0)
    {
        wchar_t str[CL_MAX_PATH];
        wcscpy_s(str, s.c_str());
...
void assertCorrectQuery(CuTest *tc, const wchar_t* query, Analyzer* a, const wchar_t * inst, const wchar_t* msg)
{
    Query* q = getQuery(tc, query, a);
...
void assertCorrectQuery(CuTest *tc, Query* q, const wchar_t* inst, bool bDeleteQuery = false)
{
...
void assertParseException(CuTest *tc, const wchar_t* queryString)
{
    try
    {
...
void assertEscapedQueryEquals(CuTest *tc, const wchar_t* query, Analyzer* a, const wchar_t* result)
{
    std::wstring escapedQuery = QueryParser::escape(query);
    if (wcscmp(escapedQuery.c_str(), result) != 0)
    {
        wchar_t str[CL_MAX_PATH];
        wcscpy(str, escapedQuery.c_str());
        CuFail(tc, _T("Query /%s/ yielded /%s/, expecting /%s/\n"), query, escapedQuery, result);
    }
    
}

Query* getQueryDOA(const wchar_t* query, Analyzer* a = NULL)
{
...
void assertQueryEqualsDOA(CuTest *tc, const wchar_t* query, Analyzer* a, const wchar_t* result)
{
...
void testSimple(CuTest *tc)
{
    ...
    assertCorrectQuery(tc, _T("a AND b"), NULL, L"BooleanQuery", _T("a AND b"));
    assertCorrectQuery(tc, _T("hello"), NULL, L"TermQuery", _T("hello"));
    assertCorrectQuery(tc, _T("\"hello there\""), NULL, L"PhraseQuery", _T("\"hello there\""));
    ...
    std::wstring s = bq->toString(_T("field"));
    if (wcscmp(s.c_str(), _T("+term1 +term2")) != 0)
    {
...
void testWildcard(CuTest *tc)
{
    ...
    assertCorrectQuery(tc, _T("term*"), NULL, L"PrefixQuery", _T("term*"));
    assertCorrectQuery(tc, _T("term*^2"), NULL, L"PrefixQuery", _T("term*^2.0"));
    assertCorrectQuery(tc, _T("term~"), NULL, L"FuzzyQuery", _T("term~0.5"));
    assertCorrectQuery(tc, _T("term~0.7"), NULL, L"FuzzyQuery", _T("term~0.7"));
    assertCorrectQuery(tc, _T("t*"), NULL, L"PrefixQuery", _T("t*"));
    ...
    assertCorrectQuery(tc, _T("term*germ"), NULL, L"WildcardQuery", _T("term*germ"));
...
void testLeadingWildcardType(CuTest *tc)
{
    ...
    assertCorrectQuery(tc, qp->parse(_T("t*erm*")), WildcardQuery::getClassName().c_str(), true);
    assertCorrectQuery(tc, qp->parse(_T("?t*erm*")), WildcardQuery::getClassName().c_str(), true); // should not throw an exception
    assertCorrectQuery(tc, qp->parse(_T("*t*erm*")), WildcardQuery::getClassName().c_str(), true);
    _CLLDELETE(qp);
}
...
void testQPA(CuTest *tc)
{
    ...
    assertQueryEquals(tc, _T("(stop)"), &qpAnalyzer, _T(""));
    assertQueryEquals(tc, _T("((stop))"), &qpAnalyzer, _T(""));
    assertCorrectQuery(tc, _T("term term term"), &qpAnalyzer, L"BooleanQuery", _T("term term term"));
    assertCorrectQuery(tc, _T("term +stop"), &qpAnalyzer, L"TermQuery", _T("term +stop"));
}
...
void testRange(CuTest *tc)
{
    ...
    assertCorrectQuery(tc, _T("[ a TO z]"), NULL, L"ConstantScoreRangeQuery", _T("[a TO z]"));

    QueryParser* qp = _CLNEW QueryParser(_T("field"), &a);
    qp->setUseOldRangeQuery(true);
    Query* q = qp->parse(_T("[ a TO z]"));
    _CLLDELETE(qp);
    CLUCENE_ASSERT(q->instanceOf(L"RangeQuery"));
    _CLLDELETE(q);
    ...
    assertCorrectQuery(tc, _T("[ a z]"), NULL, L"ConstantScoreRangeQuery", _T("[ a z]"));
    assertQueryEquals(tc, _T("[ a z ]"), NULL, _T("[a TO z]"));
    assertQueryEquals(tc, _T("{ a z}"), NULL, _T("{a TO z}"));
    assertQueryEquals(tc, _T("{ a z }"), NULL, _T("{a TO z}"));
...
wchar_t* getDate(int64_t d, DateTools::Resolution resolution)
{
...
/** for testing DateTools support */
wchar_t* getDate(const wchar_t* s, DateTools::Resolution resolution)
{
    return getDate(DateTools::stringToTime(s), resolution);
}

void assertDateRangeQueryEquals(CuTest* tc, QueryParser* qp, const wchar_t* field,
    const wchar_t* startDate, const wchar_t* endDate,
    int64_t endDateInclusive, DateTools::Resolution resolution)
{
    ...
    std::wstring result;
    result.push_back(L'[');
    wchar_t* tmp = getDate(startDate, resolution);
    result.append(tmp);
    ...
    assertQueryEquals(tc, qp, field, query.c_str(), result.c_str());
...
wchar_t* getLocalizedDate(int32_t year, int32_t month, int32_t day, bool extendLastDate)
{
    if (extendLastDate)
...
void testDateRange(CuTest* tc)
{
    ...
    wchar_t* startDate = getLocalizedDate(2002, 1, 1, false);
    wchar_t* endDate = getLocalizedDate(2002, 1, 4, false);
    const int64_t endDateExpected = CL_NS(document)::DateTools::getTime(2002, 1, 4, 23, 59, 59, 999);
    const wchar_t* defaultField = _T("default");
    const wchar_t* monthField = _T("month");
    const wchar_t* hourField = _T("hour");
...
{
    const wchar_t* stopWords[] = { _T("on"), NULL };
    StandardAnalyzer* oneStopAnalyzer = _CLNEW StandardAnalyzer(reinterpret_cast<const wchar_t**>(&stopWords));
    ...
    CLUCENE_ASSERT(q != NULL);
    std::wstring tmp = q->toString();
    CLUCENE_ASSERT(wcscmp(tmp.c_str(), _T("")) == 0);
...
void testMatchAllDocs(CuTest *tc)
{
    WhitespaceAnalyzer a;
    QueryParser* qp = _CLNEW QueryParser(_T("field"), &a);
    assertCorrectQuery(tc, qp->parse(_T("*:*")), L"MatchAllDocsQuery", true);
    assertCorrectQuery(tc, qp->parse(_T("(*:*)")), L"MatchAllDocsQuery", true);
    ...
    assertCorrectQuery(tc, clauses[0]->getQuery(), L"MatchAllDocsQuery");
    assertCorrectQuery(tc, clauses[1]->getQuery(), L"MatchAllDocsQuery");
...
void testDefaultField(CuTest* tc)
{
    ...
    std::wstring s = bq->toString(_T("field"));
    _CLLDELETE(bq);
    if (wcscmp(s.c_str(), _T("term1 author:term2 term3")) != 0)
    ...
    if (wcscmp(s.c_str(), _T("term1 *:term2 term3")) != 0)
