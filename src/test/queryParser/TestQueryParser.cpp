...
void assertCorrectQuery(CuTest *tc,const wchar_t* query, Analyzer* a, const char* inst, const wchar_t* msg){
	Query* q = getQuery(tc,query,a);
...
void assertEscapedQueryEquals(CuTest *tc,const wchar_t* query, Analyzer* a, const wchar_t* result){
	wchar_t* escapedQuery = QueryParser::escape(query);
	if (wcscmp(escapedQuery, result) != 0) {
		wchar_t str[CL_MAX_PATH];
		wcscpy(str,escapedQuery);
...
void testSimple(CuTest *tc) {
	...
	wchar_t* s = bq->toString(_T("field"));
	if ( wcscmp(s,_T("+term1 +term2")) != 0 ) {
...
void testBoost(CuTest *tc){
	...
	CLUCENE_ASSERT(q != NULL);
	wchar_t* tmp = q->toString();
	CLUCENE_ASSERT( wcscmp(tmp, _T("")) == 0 );
	_CLDELETE_LCARRAY(tmp);
...
void testDefaultField(CuTest* tc){
    ...
    wchar_t* s = bq->toString(_T("field"));
    _CLLDELETE(bq);
    if ( wcscmp(s,_T("term1 author:term2 term3")) != 0 )
    ...
    if ( wcscmp(s,_T("term1 *:term2 term3")) != 0 )
