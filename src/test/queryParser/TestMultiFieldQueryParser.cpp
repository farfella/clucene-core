...
void assertQueryEquals(CuTest *tc, const wchar_t* result, Query* q)
{
    if (q == NULL)
        return;

    std::wstring s = q->toString();
    int ret = wcscmp(s.c_str(), result);
    _CLDELETE(q);
    if (ret != 0)
    {
        wchar_t buf[HUGE_STRING_LEN];
        _snwprintf(buf, HUGE_STRING_LEN, _T("FAILED Query yielded /%s/, expecting /%s/\n"), s, result);
...
void assertStopQueryEquals(CuTest *tc, const wchar_t* qtxt, const wchar_t* expectedRes)
{
    const wchar_t* fields[] = { _T("b"), _T("t"), NULL };
    ...
    q = MultiFieldQueryParser::parse(qtxt, reinterpret_cast<const wchar_t**>(&fields),
