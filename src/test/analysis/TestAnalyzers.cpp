...
// Ported from Java Lucene tests
  ...
  void assertAnalyzesTo(CuTest *tc,Analyzer* a, const wchar_t* input, const wchar_t* output){
   Reader* reader = _CLNEW StringReader(input);
	TokenStream* ts = a->tokenStream(_T("dummy"), reader );

	const wchar_t* pos = output;
	wchar_t buffer[80];
	const wchar_t* last = output;
	CL_NS(analysis)::Token t;
	while( (pos = wcsstr(pos+1, _T(";"))) != NULL ) {
		int32_t len = (int32_t)(pos-last);
		wcsncpy(buffer,last,len);
		buffer[len]=0;

		CLUCENE_ASSERT(ts->next(&t)!=NULL);
		CLUCENE_ASSERT( t.termLength() == wcslen(buffer) );
		CLUCENE_ASSERT(wcscmp( t.termBuffer(), buffer) == 0 );
  ...
  void assertReusableAnalyzesTo(CuTest *tc,Analyzer* a, const wchar_t* input, const wchar_t* output){
      Reader* reader = _CLNEW StringReader(input);
  ...
  void useKeywordTokenizer(CuTest *tc, const wchar_t* text) {
  ...
  void testPayloadCopy(CuTest* tc) {
      const wchar_t* s = _T("how now brown cow");
      TokenStream* ts;
  ...
  // StandardAnalyzer constants remain publicly accessible
   void _testStandardConstants() {
       ...
       // const wchar_t** y = standard::tokenImage; <-- TODO: Failing compile time test
   }

// TestPerFieldAnalzyerWrapper.Java
   void testPerFieldAnalzyerWrapper(CuTest *tc){
        const wchar_t* text = _T("Qwerty");
...
#define SUB_ANALYZER_TYPE lucene::analysis::standard::StandardAnalyzer
  ...
  void testISOLatin1AccentFilter(CuTest *tc){
	  wchar_t str[200];
	  wcscpy(str, _T("Des mot cl\xe9s \xc0 LA CHA\xceNE \xc0 \xc1 \xc2 ") //Des mot cl?s ? LA CHA?NE ? ? ? 
  ...
  void testWordlistLoader(CuTest *tc){
	  ...
	  wchar_t testString[10];
	  wcscpy(testString, _T("test"));
