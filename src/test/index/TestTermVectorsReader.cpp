...
CL_NS_USE(util);
  ...
  const wchar_t* testFields_values[] = {_T("f1"), _T("f2"), _T("f3"), _T("f4")};
  ...
  std::vector<const wchar_t*> testFields(testFields_values, testFields_values + sizeof(testFields_values) / sizeof(wchar_t*));
  std::vector<const wchar_t*> testTerms(testTerms_values, testTerms_values + sizeof(testTerms_values) / sizeof(wchar_t*));
  ...
  public:
      virtual TokenStream* tokenStream(const wchar_t* fieldName, Reader* reader) {
  ...
  struct Mywchar_tCompare :
    public std::binary_function<const wchar_t*, const wchar_t*, bool>
  {
  ...
  void setUp() {
    ...
    tokens.clear();
    std::sort(testTerms.begin(), testTerms.end(), Mywchar_tCompare());
    int tokenUpto = 0;
    for (std::vector<const wchar_t*>::size_type i = 0; i < testTerms.size(); i++) {
      ...
      for (int j = 0; j < TERM_FREQ; j++) {
        ...
        offsets[i][j] = _CLNEW TermVectorOffsetInfo(j * 10, j * 10 + wcslen(testTerms[i]));
    ...
    for(std::vector<const wchar_t*>::size_type i=0;i<testFields.size();i++) {
      Field::TermVector tv;
  ...
  void testTermVectorsReader(CuTest* tc) {
    ...
    for (int j = 0; j < 5; j++) {
      ...
      for (int i = 0; i < terms->length; i++) {
        const wchar_t* term = (*terms)[i];
        CuAssertStrEquals(tc, _T(""), testTerms[i], (wchar_t*)term, false);
      }
    }
  }
  ...
  void testPositionReader(CuTest* tc) {
    ...
    const ArrayBase<const wchar_t*>* terms;
  ...
  public:
    ...
    virtual void setExpectations(const wchar_t* _field, const int32_t numTerms, const bool storeOffsets, const bool storePositions) {
      if (documentNumber == -1) {
        _CLTHROWA(CL_ERR_Runtime, "Documentnumber should be set at this point!");
      }
    }

    virtual void map(const wchar_t* term, const int32_t termLen, const int32_t frequency, CL_NS(util)::ArrayBase<TermVectorOffsetInfo*>* offsets, CL_NS(util)::ArrayBase<int32_t>* positions) {
      if (documentNumber == -1) {
        _CLTHROWA(CL_ERR_Runtime, "Documentnumber should be set at this point!");
      }
    }
  ...
  void testOffsetReader(CuTest* tc) {
    ...
    CuAssertTrue(tc, vector != NULL, _T("Term position vector expected!"));
    const CL_NS(util)::ArrayBase<const wchar_t*>* terms = vector->getTerms();
    CuAssertTrue(tc, terms != NULL, _T("Terms expected"));
    CuAssertTrue(tc, terms->length == testTerms.size(), _T("Unexpected number of terms!"));
    for (int i = 0; i < terms->length; i++) {
      const wchar_t* term = (*terms)[i];
