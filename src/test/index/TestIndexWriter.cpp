...
void IWlargeScaleCorrectness_tester::invoke(
	...
	for (value0 = 0; value0 < documents * repetitions; value0++) {
		...
		wchar_t* value0_string = NumberTools::longToString(value0 % documents);
		wchar_t* value1_string = NumberTools::longToString(value1);
	...
	for (long i = 0; i < hits0->length(); i++) {
		...
		wchar_t const* value = retrieved.get(_T("field0"));
...
void testWickedLongTerm(CuTest *tc) {
    ...
    // a too-long term:
    wchar_t* contents = _CL_NEWARRAY(wchar_t, 17000);
    wcscpy(contents, _T("abc xyz x"));
...
void testDeleteDocument(CuTest* tc) {
    ...
    for (int i = 0; i < size; i++) {
        Document* doc = _CLNEW Document();
        wchar_t* contents = _CL_NEWARRAY(wchar_t, (size / 10) + 1);
