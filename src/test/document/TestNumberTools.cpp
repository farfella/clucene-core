...
#include "test.h"
    ...
    void subtestTwoLongs(CuTest *tc, int64_t i, int64_t j) {
        // convert to strings
		wchar_t* a = NumberTools::longToString(i);
		wchar_t* b = NumberTools::longToString(j);

        // are they the right length?
		CuAssertTrue(tc, NumberTools::STR_SIZE == wcslen(a));
		CuAssertTrue(tc, NumberTools::STR_SIZE == wcslen(b));

        // are they the right order?
        if (i < j) {
            CuAssertTrue(tc, wcscmp(a,b) < 0);
        } else if (i > j) {
            CuAssertTrue(tc, wcscmp(a,b) > 0);
        } else {
            CuAssertTrue(tc, wcscmp(a,b) == 0);
    ...
    void testMin(CuTest *tc) {
		...
		CuAssertTrue(tc, LUCENE_INT64_MIN_SHOULDBE == NumberTools::stringToLong(const_cast<wchar_t*>(NumberTools::MIN_STRING_VALUE)));
		wchar_t* actual = NumberTools::longToString(LUCENE_INT64_MIN_SHOULDBE);
    ...
    void testMax(CuTest *tc) {
        // make sure the constants convert to their equivelents
		CuAssertTrue(tc, LUCENE_INT64_MAX_SHOULDBE == NumberTools::stringToLong(const_cast<wchar_t*>(NumberTools::MAX_STRING_VALUE)));
		wchar_t* actual = NumberTools::longToString(LUCENE_INT64_MAX_SHOULDBE);
