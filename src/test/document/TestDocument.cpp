...
public:
  ...
  void TestDateTools(CuTest *tc) {
	  wchar_t* t = CL_NS(document)::DateTools::timeToString( Misc::currentTimeMillis() , CL_NS(document)::DateTools::MILLISECOND_FORMAT);
	  _CLDELETE_ARRAY(t);

	  wchar_t buf[30];
	  const wchar_t* xpt = _T("19700112102054321");
  ...
  */
  ...
  void TestFieldSelectors(CuTest *tc){
    RAMDirectory dir;
    const wchar_t* longStrValue = _T("too long a field...");
    {
    ...
    uint32_t shouldBeInt = 2 * wcslen(longStrValue);
  ...
  void _TestDocumentWithOptions(CuTest *tc, int storeBit, FieldSelector::FieldSelectorResult fieldSelectorBit){
    ...
    Document doc;
    Field* f;
    const wchar_t *_ts, *_ts2;
    const ValueArray<uint8_t>* strm;
    RAMDirectory ram;

    const char* areaderString = "a binary field";
    const wchar_t* treaderString = _T("a string reader field");
