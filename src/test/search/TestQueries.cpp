...
void testPrefixQuery(CuTest *tc){
	...
	const wchar_t* categories[] = {_T("/Computers"), _T("/Computers/Mac"), _T("/Computers/Windows")};
...
private:
	...
	void addDoc(const wchar_t* text, IndexWriter* writer) {
		Document* doc = _CLNEW Document();
	...
	Hits* searchQuery(IndexSearcher* searcher, const wchar_t* field, const wchar_t* text,
	...
	size_t getHitsLength(IndexSearcher* searcher, const wchar_t* field, const wchar_t* text,
		float_t minSimilarity=FuzzyQuery::defaultMinSimilarity, size_t prefixLen=0){
