...
typedef StringMap<wchar_t*, float_t> sortScores;
typedef std::pair<wchar_t*, float_t> scorePair;
...
void sortMatches(CuTest *tc, Searcher* searcher, Query* query, Sort* sort, const wchar_t* expectedResult)
{
    ...
    for (int i = 0; i < n; ++i)
    {
        ...
        wchar_t** v = doc.getValues(_T("tracer")); //todo: should be const???
    ...
    CuAssertStrEquals(tc, _T("tracer value"), expectedResult, buff.c_str());
...
void sortMatchesPattern(CuTest* tc, Searcher* searcher, Query* query, Sort* sort, const wchar_t* pattern)
{
    Hits* result = searcher->search(query, sort);
    int32_t n = result->length();
    std::wstring buff;
    ...
    {
        Document& doc = result->doc(i);
        wchar_t** v = doc.getValues(_T("tracer"));
        for (int j = 0; v[j] != NULL; ++j)
        {
            buff.append(v[j]);
        }
    ...
    CuMessage(tc, _T("\nmatching \"%s\" against pattern \"%s\""), buff.c_str(), pattern);
...
void sort_runMultiSorts(CuTest* tc, Searcher* multi)
{
    ...
    const wchar_t* sorts3[3] = { _T("int"),_T("float"),NULL };
    _sort->setSort(sorts3);
    sortMatches(tc, multi, sort_queryA, _sort, _T("IDHFGJEABC"));

    const wchar_t* sorts4[3] = { _T("float"),_T("string"),NULL };
...
{
    const wchar_t* sorts1[3] = { _T("int"),_T("float"), NULL };
    _sort->setSort(sorts1);
    ...
    const wchar_t* sorts3[3] = { _T("float"),_T("string"), NULL };
    _sort->setSort(sorts3);
