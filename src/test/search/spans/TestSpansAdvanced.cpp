...
const wchar_t * TestSpansAdvanced::field_id = _T( "ID" );
const wchar_t * TestSpansAdvanced::field_text = _T( "TEXT" );
...
void TestSpansAdvanced::addDocument( IndexWriter * writer, const wchar_t * id, const wchar_t * text )
{
    Document document;
...
void TestSpansAdvanced::doTestBooleanQueryWithSpanQueries( const float_t expectedScore )
{
    ...
    const wchar_t * expectedIds[] = { _T( "1" ), _T( "2" ), _T( "3" ), _T( "4" ) };
...
void TestSpansAdvanced::assertHits( Query * query, const wchar_t * description, const wchar_t ** expectedIds, float_t * expectedScores, size_t expectedCount )
{
    ...
    *****/
    ...
    for( size_t i = 0; i < expectedCount; i++ )
    {
        ...
        Document doc;
        searcher->doc( id, doc );
        assertTrueMsg( _T( "actual document does not match the expected one" ), 0 == wcscmp( expectedIds[ i ], doc.get( field_id )));
