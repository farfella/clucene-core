...
void TestBasics::testSpanNot()
{
    ...
    clauses[ 0 ] = _CLNEW SpanTermQuery( term1 );
    clauses[ 1 ] = _CLNEW SpanTermQuery( term2 );
    SpanNearQuery * near_ = _CLNEW SpanNearQuery( clauses, clauses+2, 4, true, true );

    SpanNotQuery * query = _CLNEW SpanNotQuery( near_, _CLNEW SpanTermQuery( term3 ), true );
...
void TestBasics::testSpanWithMultipleNotSingle()
{
    ...
    SpanNotQuery * query = _CLNEW SpanNotQuery( near_, orQuery, true );
...
void TestBasics::testNpeInSpanNearWithSpanNot()
{
    ...
    SpanNotQuery * query = _CLNEW SpanNotQuery( near_, exclude, true );
...
void TestBasics::testNpeInSpanNearInSpanFirstInSpanNot()
{
    ...
    SpanNearQuery * near_ = _CLNEW SpanNearQuery( clauses, clauses+2, n-1, true, true );
    SpanFirstQuery * exclude = _CLNEW SpanFirstQuery( near_, n-1, true );
