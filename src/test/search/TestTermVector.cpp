...
void testKnownSetOfDocuments(CuTest *tc) {
    const wchar_t* test1 = _T("eating chocolate in a computer lab"); //6 terms
    const wchar_t* test2 = _T("computer in a computer lab"); //5 terms
    const wchar_t* test3 = _T("a chocolate lab grows old"); //5 terms
    const wchar_t* test4 = _T("eating chocolate with a chocolate lab in an old chocolate colored computer lab"); //13 terms

    typedef std::map<const wchar_t*, int32_t, __wchar_tCompare> test4MapType;
    test4MapType test4Map;
    test4Map.insert( std::pair<const wchar_t*,int32_t>(_T("chocolate"), 3) );
    test4Map.insert( std::pair<const wchar_t*,int32_t>(_T("lab"), 2) );
    test4Map.insert( std::pair<const wchar_t*,int32_t>(_T("eating"), 1) );
    test4Map.insert( std::pair<const wchar_t*,int32_t>(_T("computer"), 1) );
    test4Map.insert( std::pair<const wchar_t*,int32_t>(_T("with"), 1) );
    test4Map.insert( std::pair<const wchar_t*,int32_t>(_T("a"), 1) );
    test4Map.insert( std::pair<const wchar_t*,int32_t>(_T("colored"), 1) );
    test4Map.insert( std::pair<const wchar_t*,int32_t>(_T("in"), 1) );
    test4Map.insert( std::pair<const wchar_t*,int32_t>(_T("an"), 1) );
    test4Map.insert( std::pair<const wchar_t*,int32_t>(_T("computer"), 1) );
    test4Map.insert( std::pair<const wchar_t*,int32_t>(_T("old"), 1) );
    ...
    try {
      ...
      while (termEnum->next() == true)
      {
        ...
        while (termDocs->next())
        {
          ...
          const ArrayBase<const wchar_t*>* terms = vector->getTerms();
          CLUCENE_ASSERT(vector != NULL);
          ...
          const ArrayBase<const wchar_t*>* vTerms = vector->getTerms();
          const ArrayBase<int32_t>* freqs = vector->getTermFrequencies();
          size_t i=0;
          while ( vTerms && i < vTerms->length )
          {
            if ( wcscmp(term->text(), vTerms->values[i]) == 0 )
            {
      ...
      for (int32_t i = 0; i < termsLength; i++) {
        const wchar_t* term = terms->values[i];
        //_tprintf(_T("Term: %s, test4map.size()=%d\n"),term, test4Map.size());
        int32_t freq = (*freqs)[i];
        CLUCENE_ASSERT( wcsstr(test4,term) != NULL );
