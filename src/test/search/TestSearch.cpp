...
#include "CLucene/search/MultiPhraseQuery.h"
	...
	void _TestSearchesRun(CuTest *tc, Analyzer* analyzer, Searcher* search, const wchar_t* qry){
		Query* q = NULL;
		Hits* h = NULL;
		try{
...
void SearchTest(CuTest *tc, bool bram) {
  ...
  writer.setMaxBufferedDocs(3);
	...
	const wchar_t* docs[] = { _T("a b c d e asdf"),
...
void testSrchManyHits(CuTest* /*tc*/) {
  SimpleAnalyzer analyzer;
	...
	const wchar_t* docs[] = { _T("a b c d e"),
