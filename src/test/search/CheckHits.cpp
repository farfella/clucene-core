...
public:
    ...
    ExplanationAsserter( Query * q, const wchar_t * defaultFieldName, Searcher * s, CuTest * tc )
    {
        this->q = q;
        this->s = s;
    ...
    ExplanationAsserter( Query * q, const wchar_t * defaultFieldName, Searcher * s, bool deep, CuTest * tc )
    {
        this->q = q;
        this->s = s;
        this->d = q->toString( defaultFieldName );
...
bool CheckHits::stringEndsWith( const wchar_t* tszStr, const wchar_t * tszEnd )
{
    if( ! tszStr || ! tszEnd )
        return false;

    size_t lenStr = wcslen( tszStr );
    size_t lenEnd = wcslen( tszEnd );
    if( lenEnd > lenStr )
        return false;

    const wchar_t * tszReadEnd = tszEnd + lenEnd;
    const wchar_t * tszReadStr = tszStr + lenStr;
...
/////////////////////////////////////////////////////////////////////////////
void CheckHits::checkHits( CuTest* tc, Query * query, const wchar_t * defaultFieldName, Searcher * searcher, int32_t * results, size_t resultsCount )
{
...
void CheckHits::checkExplanations( CuTest* tc, Query * query, const wchar_t * defaultFieldName, Searcher * searcher )
{
    checkExplanations( tc, query, defaultFieldName, searcher, false );
}

void CheckHits::checkExplanations( CuTest* tc, Query * query, const wchar_t * defaultFieldName, Searcher * searcher, bool deep )
{
        ...
        else 
        {
            ...
            if( ! ( productOf || sumOf || maxOf ))
            {
                ...
                if( k1 )
                {
                    ...
                    const wchar_t * k2 = wcsstr( k1, _T( " " ));

                     x = _tcstod( k1, NULL );   // Float::parseFloat( descr.substring(k1,k2).trim());

                    if( k2 && 0 == wcscmp( k2, _T( " times others of:" ))) 
                        maxTimesOthers = true;
                }
            }
