 ...
 * CuTcs
 *-------------------------------------------------------------------------*/
wchar_t* CuStrAlloc(int size)
{
	wchar_t* n = (wchar_t*) malloc( sizeof(wchar_t) * (size) );
	return n;
}

wchar_t * CuTcsCopy(const wchar_t * old)
{
    wchar_t * n = NULL;
	size_t len = wcslen(old);
    if (len >= 0)
    {
        n = CuStrAlloc(len + 1);
        if (n != NULL)
        {
            wcscpy_s(n, len + 1, old);
        }
    }
...
wchar_t * CuTcsCat(wchar_t * orig, wchar_t * add)
{
    wchar_t * n = NULL;
	size_t len = wcslen(orig) + wcslen(add);
    if (len >= 0)
    {
        n = CuStrAlloc(len + 1);
        if (n != NULL)
        {
            wcscpy_s(n, len + 1, orig);
            wcscat_s(n, len + 1, add);
        }
    }
	return n;
}
...
void CuStringInit(CuString* str)
{
	str->length = 0;
	str->size = STRING_MAX;
	str->buffer = (wchar_t*) malloc(sizeof(wchar_t) * str->size);
	str->buffer[0] = '\0';
}
...
{
	str->buffer = (wchar_t*) realloc(str->buffer, sizeof(wchar_t) * newSize);
	str->size = newSize;
}

void CuStringAppend(CuString* str, const wchar_t* text)
{
...
void CuStringAppendFormat(CuString* str, const wchar_t* format, ...)
{
	wchar_t buf[HUGE_STRING_LEN];
	va_list argp;
...
void CuStringRead(CuString *str, wchar_t *path)
{
	path = NULL;
...
void CuTestInit(CuTest* t, const wchar_t* name, TestFunction function)
{
...
CuTest* CuTestNew(const wchar_t* name, TestFunction function)
{
...
void CuNotImpl(CuTest* tc, const wchar_t* message)
{
...
void CuFail(CuTest* tc, const wchar_t* format, ...)
{
	tc->failed = 1;
...
void CuMessageV(CuTest* tc, const wchar_t* format, va_list& argp){
	wchar_t buf[HUGE_STRING_LEN];
	_vsntprintf(buf, HUGE_STRING_LEN, format, argp);
...
void CuMessage(CuTest* tc, const wchar_t* format, ...){
	va_list argp;
	va_start(argp, format);
...
void CuAssert(CuTest* tc, const wchar_t* message, int condition)
{
	if (condition) return;
	CuFail(tc, message);
}

void CuAssertTrue(CuTest* tc, int condition, const wchar_t* msg)
{
	if (condition) return;
    if (msg != NULL)
	    CuFail(tc, msg);
...
void CuAssertEquals(CuTest* tc, const int32_t expected, const int32_t actual, const wchar_t* msg)
{
    CuAssertIntEquals(tc, msg, expected, actual);
}

void CuAssertStrEquals(CuTest* tc, const wchar_t* preMessage, const wchar_t* expected, wchar_t* actual, bool bDelActual){
  CuString* message;
...
void CuAssertStrEquals(CuTest* tc, const wchar_t* preMessage, const wchar_t* expected, const wchar_t* actual)
{
	CuString* message;
	if (wcscmp(expected, actual) == 0) {
		return;
...
void CuAssertIntEquals(CuTest* tc, const wchar_t* preMessage, int expected, int actual)
{
    if (expected == actual) return;
    wchar_t buf[STRING_MAX];
    if (preMessage != NULL){
        _snwprintf(buf, STRING_MAX, _T("%s : expected <%d> but was <%d>"), preMessage, expected, actual);
    } else {
        _snwprintf(buf, STRING_MAX, _T("Assert failed : expected <%d> but was <%d>"), expected, actual);
    }
    CuFail(tc, buf);
}
void CuAssertSizeEquals(CuTest* tc, const wchar_t* preMessage, size_t expected, size_t actual)
{
    if (expected == actual) return;
    wchar_t buf[STRING_MAX];
    if (preMessage != NULL){
...
void CuAssertPtrEquals(CuTest* tc, const wchar_t* preMessage, const void* expected, const void* actual)
{
	wchar_t buf[STRING_MAX];
	if (expected == actual) return;
	_snwprintf(buf, STRING_MAX,_T("%s : expected pointer <%p> but was <%p>"), preMessage, expected, actual);
	CuFail(tc, buf);
}

void CuAssertPtrNotNull(CuTest* tc, const wchar_t* preMessage, const void* pointer)
{
	wchar_t buf[STRING_MAX];
	if (pointer != NULL ) return;
	_snwprintf(buf,STRING_MAX, _T("%s : null pointer unexpected, but was <%p>"), preMessage, pointer);
	CuFail(tc, buf);
}
...
void CuSuiteInit(CuSuite* testSuite, const wchar_t *name)
{
	testSuite->name = NULL;
...
CuSuite* CuSuiteNew(const wchar_t *name)
{
...
CuSuiteList* CuSuiteListNew(const wchar_t *name)
{
...
static const wchar_t *genspaces(int i)
{
    wchar_t *str = (wchar_t*)malloc((i + 1) * sizeof(wchar_t));
    for ( int j=0;j<i;j++ )
		str[j]=_T(' ');
    str[i] = '\0';
    return str;
...
void CuSuiteListRunWithSummary(CuSuiteList* testSuite, bool verbose, bool times)
{
	...
	for (i = 0 ; i < testSuite->count ; ++i)
	{
		...
		size_t len = wcslen(testCase->name);
		const wchar_t* spaces = len>31?NULL:genspaces(31 - len);
			...
			for ( int i=0;i<testCase->count;i++ ){
				if ( testCase->list[i]->ran ){
					if ( testCase->list[i]->message != NULL ){
						...
						wchar_t* msg = testCase->list[i]->message;
						bool nl = true;
						...
						if ( testCase->list[i]->message[wcslen(testCase->list[i]->message)-1] != '\n' )
							printf("\n");
						hasprinted=true;
					}
				}
